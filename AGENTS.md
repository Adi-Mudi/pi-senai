...
│   ├── mission-packs.ts      # per-mission-type packs (bugfix = strict; others EMPTY_PACK)
│   ├── compaction-summary.ts # session_before_compact payload
│   └── agents-config/        # Senai config domain incl. agent discovery (moved from agents/ — Layer 0 so Layer 1 can import it)
│
├── io/                       # I/O layer
...
│
│   (the architecture factory moved to the @adi-mudi/pi-chirpi package —
│    senai imports its API and doctor checks from there)
│
...
│   ├── checks-generated-team.ts # generated team agents, generator completeness, technology resources
...
│   └── checks-gates.ts       # hard enforcement gates (spawn guard wiring, web tool lock, stage machine, layer map, framework library + gate wiring, cadence sanity)
│
...
│   ├── signals.ts            # collect + format + persist verification reports
│   ├── manifest.ts           # ## Files manifest parser + diff (planned vs disk)
│   ├── arch-rules-check.ts   # emitted dependency-cruiser / import-linter config runner
│   ├── framework-rules-check.ts # locked framework ## Rules runner (same mechanism)
...
│
│   (the architect commands moved to @adi-mudi/pi-chirpi:
│    /chirpi-configure-architect-inputs, /chirpi-generate-architect, /chirpi-predefined-architect)
```

There are no re-export shims. The old flat `src/commands.ts` and
`src/architect.ts` were split into the layered folders above; the
architecture factory itself has since moved to `@adi-mudi/pi-chirpi`.

**Test layout mirrors src/:** `test/core/`, `test/io/`, `test/agents/`, `test/doctor/`, `test/docs-factory/`, `test/implement/`, `test/ui/`. E2E tests live in `test/e2e/`.
...
8. **Testing discipline.** The Implement stage enforces a testing discipline across all four stages. The deterministic scanner (`pi-extension/src/implement/discipline.ts`) flags 11 anti-patterns (`zero-assertion`, `over-mocking`, `mirror-logic`, `flaky-timing`, `no-aaa`, `mystery-guest`, `private-method`, `god-test`, plus the NEVER-list heuristics `deleted-test-file`, `weakened-assertion`, `swallowed-error`) — the first two are blocking; the rest are actionable or informational by default and escalate to blocking for bugfix missions (strict-mode gating). Bugfix missions additionally get an unconditional (confirm-overridable) approve-gate signal, `bugfixRegressionTest`: the brief's `## Regression test plan` must name a test file that exists on disk and is covered by the plan's `## Files` manifest. The scanner is exposed as the `senai_scan_test_smells` tool (registered in `pi-extension/src/index.ts`). The Implement stage's approval gate collects three signals before prompting the user: scan report + coverage (from `coverage/coverage-summary.json` if present) + mission verification re-run (parses `<plan>` `## Verification` and runs each step via `bash`, preceded by baseline steps auto-derived from the project's own `package.json` — `npm run build` / `npm test` always run, planner-independent). Two hard gates sit on top: `/senai-approve` at `planning` refuses to advance when the plan has zero executable verification steps, and failed verification steps always surface as blocking reasons. The same gates also enforce the plan's `## Files` manifest diff, the emitted arch-rules config, and the framework lock: the manifest's optional `## Framework` field must match the profiler-detected or tech-stack-named framework, and at the implement gate the locked framework's `## Rules` run via `pi-extension/src/implement/framework-rules-check.ts` (report at `implement/framework-rules-report.md`; `none` skips cleanly). Strict mode is OFF by default (`SENAI_TEST_DISCIPLINE_STRICT=1` opts in). Coverage floor defaults to 80% (`SENAI_TEST_DISCIPLINE_COVERAGE_FLOOR` overrides). The generated `test-skeleton` agent also scaffolds one narrow integration test per critical path in the plan's `## Verification` section, and the generated `full-test` agent must report an explicit "no integration tests exist" finding instead of silently passing. The discipline shows up in five places: the three stage skills (`senai-implement.md`, `senai-document.md`, `senai-deliver.md`), the generated test-related agent bodies (`<project>-test-skeleton.md`, `<project>-linter.md`, `<project>-full-test.md`, all four doc-writers), the architecture-generated `implementer` / `reviewer-tests` / `reviewer-correctness` agents, and the two new `/senai-doctor` sections ("Testing discipline" + "Sub-agent generator completeness"). `GENERATOR_VERSION` is currently 10 — bumping it makes the existing doctor check flag stale generated agents as warnings so users regenerate and pick up the new sections.
...
| Location | Purpose |
|---|---|
| `.pi/agents/<project>-<architecture-id>-<role>.md` | Six generated architecture agents (generator v10 added reviewer-architecture). |
...
- `senai_finalize_architecture` — generates docs, agents, and skills with exact names. Removes agents and skills left over from previous architecture runs only when the generation manifest proves pi-senai wrote them and the user never edited them (name pattern + sha256 guard); anything else is kept and reported as `keptStaleArtifacts`. Regenerates the ADR set to match the report, and preserves sub-agent entries in `generated-manifest.json` (merge, not wipe) so team-agent drift tracking survives an architect re-run. Generated architecture agents declare `session-mode: lineage-only`, `auto-exit: true`, and `spawning: false`; reviewer agents keep `write` for their review artifact but do not get the `edit` tool. Also auto-maps the eight architecture-bound roles in `agents.json` (creating the file if missing) via `autoMapArchitectureAgents`: roles on built-in defaults or pointing at stale generated agents for the same project are remapped; other custom mappings are never touched. The role→agent mapping lives in `ARCHITECTURE_AGENT_MAPPING` in the `@adi-mudi/pi-chirpi` package (the architecture factory moved there), which doctor also uses.

## Sub-agent generation

`/senai-generate-sub-agents` deterministically generates sub-agents for the 15 non-architecture roles (the 8 architecture-bound roles belong to the architecture factory). Agent content is assembled, never LLM-generated: role template + technology resource + architect report context. Generated agents declare `session-mode: lineage-only`, `auto-exit: true`, and `spawning: false`; artifact-writing roles (scout-2/3/4, discussion, plan-overview, security-gate, and since generator v3 also linter and full-test, which write report artifacts) carry the `write` tool; and every generated body ends with a completion contract (final message ≤ 10 lines: outcome + artifact path, never pasted content). Doc-writer roles (readme-writer, changelog-writer, api-docs-writer, other-docs-writer) also carry a documentation contract from the doc catalog: default target path, template id, required sections, and the hard length cap. Since generator v10, `test-skeleton` and `full-test` also carry the framework contract: test-skeleton places and names test files by the locked framework's conventions, and full-test uses the framework's standard test runner when the plan locks a framework.
...
`state.json` schema (version 1): `version`, `mission`, `runId`, `currentStage`, `startedAt`, `updatedAt`, `stageResults`, plus (since the discussion entry point) the optional fields `discussions: number`, `discussionEvents: Array<{ts, transcriptPath, briefPath, afterStage?}>`, and `missionBriefPath?: string`. Brainstorm lifecycle v2 adds the optional fields `brainstormRunId?: string`, `understandingConfirmed?: boolean`, `missionType?: "bugfix" | "feature" | "upgrade" | "explore" | "docs" | "test"`, `scansSelected?: Array<"code" | "doc" | "community">`, `brainstormQuestions?: BrainstormQuestion[]`, `exploreDecision?: "go" | "clarify" | "kill"` (explore only — the decide door, required before approve finalizes an explore mission), and `discussionDocPath?: string`. Per-type stage sequences, default scans, and question packs live in the `MISSION_SEQUENCE` registry (`pi-extension/src/brainstorm/registry.ts`) — the single source of truth the skill file and commands read. Legacy v1 states load with these fields undefined; `defaultState()` seeds `discussions: 0` and `discussionEvents: []` for fresh runs.
...
Stage transitions are defined in `core/paths.ts` as `STAGE_TRANSITIONS`.
...
- `core/agents-config/discovery.ts` — walks the project tree, reads user agents, and appends built-in defaults (moved from `agents/discovery.ts` to Layer 0 so Layer 1 modules can import it).
