...
- Six mission types with per-type sequences: `bugfix` / `feature` / `upgrade` / `explore` / `docs` / `test`, driven by the new `MISSION_SEQUENCE` registry (`brainstorm/registry.ts`) — each type carries its own stage list, default scans, and question pack. `explore` ends at a DECIDE door (go / clarify / kill) via the new `set-decision` tool action and `state.exploreDecision`; `/senai-plan` refuses to adopt explore brainstorms, and a finalized explore session auto-resets at the next `/senai-brainstorm`. `docs` and `test` keep the plan road but skip the document stage.
- Brainstorm read-only gate: the `tool_call` hook now hard-blocks `edit` / `write` outside the brainstorm folder while a new-flow brainstorm is open (fail-open, legacy-exempt, lifts on finalize).
- Session-portable brainstorm state: session-tool mutations append `senai-brainstorm` entries to the pi session file (`/fork` and `/resume` carry progress); `session_start` surfaces an in-flight brainstorm even at stage `none`.
...
- `/senai-brainstorm` lifecycle v2 — the conversational pass is now a fixed sequence: [0] UNDERSTAND with dynamic, mission-type-based questions (`bugfix` / `feature` / `upgrade` packs) → [1] CONFIRM loop hard-locked by `state.understandingConfirmed` (the injected prompt is sealed — no registry or file tree — until the user confirms) → [2] one scan-selection gate over three scan types (code / doc / community, per-mission-type defaults) → [3] parallel scans (community 90s timeout, code/doc 30s, max 2 roles per scan type via `SCAN_TYPE_ROLES`) → [4] INFORM with suggested default answers → [5] DISCUSS loop with question states `draft / discussing / agreed / not-wanted / replaced` → [6] structured batch confirmation → [7] show-only coverage table → [8] approve with go / clarify / kill outcomes and a continue / compact / fresh-session handoff choice. Sessions started before this change (no `missionType`) keep the legacy flow. Every decision is synced into `discussion-NN-<slug>.md` under a `## Decisions` block, and `not-wanted` questions land in the brief's `## Out-of-scope` with their reason so the Plan stage sees both sides. Compaction summaries now carry the brainstorm handoff (run ID, discussion document path), and `/senai-plan` copies the newest discussion document into the run folder on adoption (`state.discussionDocPath`).
- Hardening baseline: run-lock acquisition no longer busy-spins (async polling + `Atomics.wait`), the `tool_call` hook is fail-open (guard crashes can no longer wedge tool calls), legacy migrations moved from the extension factory into the `session_start` hook with `ctx.hasUI` guards, and the four configure/generate commands refuse their pickers in non-interactive sessions.
...
- New `senai_brainstorm_session` tool (`brainstorm/state-tool.ts`) — the parent LLM's bridge into brainstorm state, with three actions: `confirm-understanding`, `set-scans`, `upsert-question`. All writes go through the file-mutation queue and the project-wide run lock.
