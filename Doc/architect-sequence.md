# Architect Sequence — `/chirpi-generate-architect` (moved to pi-chirpi)

> **Moved:** the architecture factory now lives in the companion package `@adi-mudi/pi-chirpi`. Commands: `/chirpi-configure-architect-inputs`, `/chirpi-generate-architect`, `/chirpi-predefined-architect`. This document is kept for the sequence design; the implementation and current behavior (including the codebase-first suggest flow) live in the pi-chirpi package.

This document describes the design and runtime sequence for the `/chirpi-generate-architect` command.
...
| Command | Purpose |
|---------|---------|
| `/chirpi-configure-architect-inputs` | Select which documents the architect agent reads. |
| `/chirpi-generate-architect` | Run the full architect generation flow. |

---

## Preconditions

Before running `/chirpi-generate-architect`:
...
If architect inputs are missing, the command tells the user to run `/chirpi-configure-architect-inputs` first. For Pi extension projects, the message suggests `/chirpi-predefined-architect` instead — the new command picks architecture from the library without requiring authored input documents.
...
### Command
```
/chirpi-predefined-architect
```

### What it does

1. Loads `.pi/architecture-library/` (36 entries bundled in v1.6.0+).
2. Profiles the codebase first (`profileCodebase`): infers purpose / scale / deployment / real-time from `package.json`, folders, and config files, each with a confidence level and evidence.
3. Asks the minimum dynamic questions (`planQuestions`): 1 confirm when signals are confident; a close-call question plus at most 2 targeted questions when the top-2 entries are close; exactly 2 fallback questions (purpose, scale) when there are no signals. Every question quotes its evidence.
4. Scores library entries deterministically (`suggestArchitectures` in `library-suggester.ts`), each suggestion carrying match reasons.
5. Shows the top 3 matches with per-match reasons; a confident close-call pick is pre-ordered first.
6. User picks one — or selects "None fit" to see the new-entry template guide.
7. Auto-discovers drivers from `package.json` + `README.md` + `.pi/agents/` + `.pi/skills/` + `.pi/extensions/` (per `createInputsConfigFromCodebase`).
8. Runs the factory with the chosen library entry as `selectedArchitecture`.
...
`/chirpi-generate-architect` suggests this command when no `architect-inputs.json` is configured AND the project is detected as a Pi extension. The doctor also surfaces the suggestion in the Library Completeness section.
...
| Problem | Action |
|---------|--------|
| Architect inputs not configured | Tell user to run `/chirpi-configure-architect-inputs`. |
...
When `/chirpi-generate-architect` runs on a project whose `package.json` imports from `@mariozechner/pi-*` or has a `pi-package` keyword, the factory enters **Pi Extension Mode**:
