...
- `/chirpi-predefined-architect` — pick architecture from the library; codebase-first profiling asks only the minimum dynamic questions; no input docs required. Falls back to a new-entry template if no match.
- `/chirpi-generate-architect` — generate the project architecture, five architecture agents, and four architecture skills; creates or updates `.pi/senai/agents.json` and auto-maps the seven architecture-bound roles. The factory ships 36 library entries (17 application architectures + 10 Pi extension sub-patterns + 6 Pi official specs + 3 Pi-aware project architectures) and enters **Pi Extension Mode** when the project imports from `@mariozechner/pi-*` or declares `pi-package`/`pi-extension` keywords. In that mode the generated architecture includes a 12-rule "Pi Extension Mandatory Rules" section.
- `/senai-doctor` — run a full diagnostic on Senai configuration, agent-role fit, file scope, runtime environment, and the architecture factory output (agent mapping, generated agent content, drift). Two new sections cover Pi extension conformance and library completeness.
