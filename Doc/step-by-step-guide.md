...
- `08-architect-tools` — removed: the architect commands moved to
  `@adi-mudi/pi-chirpi` and are tested there.
- `09-discussion` — pins the pre-run `mission-brief.md` shape consumed by
  `/senai-plan`.
- `10`–`27` — cadence, brainstorm, doctor, docs-factory, generator, and
  implement-signals coverage; see the files under `test/e2e/` for details.
- `28-plan-verification-gate` — the planning hard gate: a plan with a
  prose-only or missing `## Verification` section cannot be approved; adding
  one executable step unblocks it.
- `29-baseline-verification` — the implement gate always runs the baseline
  steps derived from `package.json` (`npm run build` / `npm test`) before the
  plan's own steps; a failing baseline blocks the advance.
- `30-generator-v9-stale-flag` — doctor flags a technology resource missing
  the `### Integration testing` subsection, and the generated-agent version
  marker matches `GENERATOR_VERSION`.
- `31-plan-files-manifest` — the planning hard gate for the `## Files`
  manifest: a plan without a valid manifest table cannot be approved; fixing
  the table unblocks it (requires a real LLM API key, skips cleanly without).
- `32-arch-rules-manifest` — the implement-stage hard fails from Phase 3:
  `signalsBlockAdvance` blocks on manifest-diff violations (planned file not
  created / unplanned file in a manifest folder) and on arch-rules violations
  when the tool ran, and only warns when the tool is absent. No LLM needed.
- `33-hard-enforcement` — the Phase-4 machine gates: an out-of-order stage
  command is rejected naming the exact command to run first, and the spawn
  guard hard-blocks a non-discussion role agent carrying WebSearch while
  allowing the discussion role. No LLM needed.
