...
The extension source is organised in 4 layers, mirroring Pi's own `core → modes → cli → main` pattern from `node_modules/@earendil-works/pi-coding-agent/docs/development.md`.
...
| 1 — Stage logic | `doctor/`, `docs-factory/`, `implement/`, `scouts/`, `brainstorm/` | Diagnostic, doc selection, implement signals, scouts, brainstorm (the architecture factory moved to `@adi-mudi/pi-chirpi`) | Layer 0 |
| 2 — Presentation | `agents/`, `ui/`, `hooks/` | Sub-agent registry + generation, UI widgets, Pi lifecycle hooks (agent discovery lives in `core/agents-config/` — Layer 0) | Layer 0, 1 |
| 3 — Composition | `commands/` | 23 slash commands + shared helpers | Layer 0, 1, 2 |
...
The Pi extension entry point. Default-exports `piSenaiExtension(pi)` and calls `register*` on each subsystem. See Pi's [`docs/extensions.md`](../../node_modules/@earendil-works/pi-coding-agent/docs/extensions.md) for the official `index.ts` pattern.
...
Tests mirror this structure under `pi-extension/test/`. Run with `npm test` (per [Pi's official testing guidance](../../node_modules/@earendil-works/pi-coding-agent/docs/development.md)). E2E tests live under `test/e2e/` and require `RUN_E2E=1` plus `pi` on PATH.
