...
// The 15 non-architecture Senai roles. The 8 architecture-bound roles are
...
export const GENERATED_ROLES: GeneratedRoleDef[] = [
  ...
  },
  {
    ...
    label: "Test Skeleton",
    tools: ["read", "write"],
    mandate: "Write failing test skeletons from the approved plan, before implementation starts. Testing discipline: AAA structure, equivalence partitioning + boundary value analysis, one naming convention, table-driven cases for repeated logic, and a property-based test per pure function. Tests must be RED (failing) when you finish — do not implement the feature. Scaffold one narrow integration test per critical path in the plan's ## Verification section; mock only true externals. When the plan locks a framework, place and name tests by that framework's conventions. Avoid: zero-assertion, mystery guest, over-mocking (>3 doubles), mirror-logic, and testing private methods.",
  ...
  },
  {
    ...
    label: "Full Test",
    tools: ["read", "bash", "write"],
    mandate: "Run the full test suite; write the results report (failures with exact error output) to the artifact path in your task. 'All tests pass' means: every assertion fired; no `.skip` / `xfail` / `it.todo` without a `// TODO(reason): re-enable in <ticket>` comment naming the blocker; no test exited without an assertion (zero-assertion). Report skipped tests with their TODO comments. Zero tests found (misconfigured test path) = a 'no tests discovered' finding — never success. No integration / e2e tests for the plan's critical paths = a 'no integration tests exist' finding — never a silent pass. When the plan locks a framework, use that framework's standard test runner.",
...
// v9: test-skeleton scaffolds integration tests per plan critical path;
//     full-test must report a "no integration tests exist" finding instead
//     of silently passing.
// v10: framework contract — test-skeleton places/names test files by the
//      locked framework's conventions; full-test runs the framework's
//      standard runner when the plan locks a framework.
