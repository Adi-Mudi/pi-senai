...
import { requiredBriefSectionsFor } from "../core/mission-brief.js";
import type { MissionType } from "../core/state.js";
 ...
 *  When a missionType is given, the section list comes from
 *  requiredBriefSectionsFor — a bugfix brief counts 10 sections, not 6. */
export function countBriefSections(brief: string, missionType?: MissionType): {
	filled: number;
	total: number;
} {
	const sections = requiredBriefSectionsFor(missionType);
	let filled = 0;
