...
import { SCAN_TYPE_ROLES } from "./registry.js";
import {
	guardArtifactPath,
	guardDispatchCount,
	type GuardResult,
 ...
 *  fetch + read) cannot complete in the 30s local-scan budget — 30s silently
 *  degraded community scans to "draft questions are simpler", which killed
 *  the external-evidence step exactly when it mattered. */
export const BRAINSTORM_COMMUNITY_DISPATCH_TIMEOUT_MS = 90_000;

/** Max dispatches per scan type at the scan gate (2 doc + 2 community
 *  parallel, etc.). Separate from BRAINSTORM_DISPATCH_CAP, which bounds the
 *  whole brainstorm. */
...
export interface DispatchRequest {
	...
	/** Optional: which scan gate this dispatch belongs to. When set, the
	 *  agent's role must serve that scan type (SCAN_TYPE_ROLES), the per-type
	 *  cap applies, and community scans get the longer timeout. */
	scanType?: ScanType;
}
...
export function prepareDispatch(
	...
	// 1b. Scan-type gate: the agent's role must serve the requested scan
	// type, and the per-type cap must not be exceeded.
	const role = resolveRoleForAgent(request.agent);
	if (request.scanType) {
		const allowedRoles = SCAN_TYPE_ROLES[request.scanType];
		if (!(allowedRoles as readonly string[]).includes(role)) {
			return {
				ok: false,
				reason:
					`Agent "${request.agent}" (role: ${role}) does not serve the "${request.scanType}" scan.\n` +
					`Allowed roles for "${request.scanType}": ${allowedRoles.join(", ")}.`,
			};
		}
	...
	// 5. Role was already resolved above (needed by the scan-type gate).
	const timeoutMs =
