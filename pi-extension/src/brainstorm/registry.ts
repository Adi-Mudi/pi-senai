...
import { discoverAgents } from "../core/agents-config/discovery.js";
import type { MissionType, ScanType } from "../core/state.js";
import { MISSION_PACKS, type MissionPack } from "../core/mission-packs.js";
import {
...
/** Which roles serve which scan type at the scan-plan gate.
 *  - code: pattern/flow search + risk/dependency audit of the codebase
 *  - doc: PRD/docs audit + architecture/big-picture read of internal docs
 *  - community: external web research
 *  planner stays available during DISCUSS (trade-offs) but is not a scanner. */
export const SCAN_TYPE_ROLES: Record<ScanType, readonly SenaiRole[]> = {
	code: ["scout-2", "scout-3"],
	doc: ["scout-4", "scout-1"],
	community: ["community-researcher"],
};

/** Stages a brainstorm mission can walk, in fixed trunk order. "decide" is the
 *  explore-only terminal door (go / clarify / kill) that replaces the
 *  approve → plan → implement road. */
export type BrainstormStage =
	| "understand"
	| "confirm"
	| "scans"
	| "discuss"
...
export interface MissionSequence {
	/** Ordered stages for this mission type. */
	readonly stages: readonly BrainstormStage[];
	/** Scan types pre-selected at the scan-plan gate (user can still change). */
	readonly defaultScans: readonly ScanType[];
	/** Which UNDERSTAND follow-up question pack the skill should use. */
	readonly questionPack: MissionType;
	/** One-line type description rendered in the sealed prompt's type list.
	 *  Lives here so the prompt has no hardcoded copy that can drift. */
	readonly summary: string;
	/** Lifecycle pack for this type — extra brief sections, conditional
	 *  scans, hard gates, implement rules, NEVER list (core/mission-packs.ts). */
	readonly pack: MissionPack;
}

/** One pipeline, custom sequence per mission type. The skill file and the
 *  brainstorm/plan commands read this map — adding a type is one entry. */
export const MISSION_SEQUENCE: Record<MissionType, MissionSequence> = {
	feature: {
		stages: ["understand", "confirm", "scans", "discuss", "approve", "plan", "implement", "document", "deliver"],
		defaultScans: ["code", "doc", "community"],
		questionPack: "feature",
		summary: "new capability (from zero, or new inside existing code). Full road.",
		pack: MISSION_PACKS.feature,
	},
	bugfix: {
		stages: ["understand", "confirm", "scans", "discuss", "approve", "plan", "implement", "document", "deliver"],
		defaultScans: ["code", "doc"],
		questionPack: "bugfix",
		summary: "something is broken. Full road.",
		pack: MISSION_PACKS.bugfix,
	},
	upgrade: {
		stages: ["understand", "confirm", "scans", "discuss", "approve", "plan", "implement", "document", "deliver"],
		defaultScans: ["code", "community"],
		questionPack: "upgrade",
		summary: "improve existing (perf, refactor, deps). Full road.",
		pack: MISSION_PACKS.upgrade,
	},
	explore: {
		stages: ["understand", "confirm", "scans", "discuss", "decide"],
		defaultScans: ["community", "doc"],
		questionPack: "explore",
		summary: "research only. Ends at the DECIDE door (go / clarify / kill). No plan, no implement.",
		pack: MISSION_PACKS.explore,
	},
	docs: {
		stages: ["understand", "confirm", "scans", "discuss", "approve", "plan", "implement", "deliver"],
		defaultScans: ["doc"],
		questionPack: "docs",
		summary: "documentation only. Plan targets .md files; no document stage.",
		pack: MISSION_PACKS.docs,
	},
	test: {
		stages: ["understand", "confirm", "scans", "discuss", "approve", "plan", "implement", "deliver"],
		defaultScans: ["code"],
		questionPack: "test",
		summary: "write tests. Plan ends with a verification run; no document stage.",
		pack: MISSION_PACKS.test,
	},
};

/** True when this mission type walks the plan → implement road after approve.
 *  explore does not — it ends at the decide door. */
export function missionUsesPlanRoad(type: MissionType): boolean {
	return MISSION_SEQUENCE[type].stages.includes("plan");
}
