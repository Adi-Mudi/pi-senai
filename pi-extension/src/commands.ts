  ...
  pi.registerCommand("orchestra-generate-architect", {
    ...
    handler: async (_args, ctx) => {
      ...
      const prompt = [
        ...
        ` `,
        drivers
          ? `Existing architectural drivers are available at .IDE_Plans/architect/architectural-drivers.json. Re-run the full flow only if the user asks for it or the inputs changed.`
...
function defaultArchitectSkill(): string {
  return [
    ...
    `3. Each subagent must write its output to .IDE_Plans/architect/architect-map/<sanitized-path>.json and nowhere else.`,
    `4. Call the orchestra_merge_architect_drivers tool to merge map outputs into .IDE_Plans/architect/architectural-drivers.json.`,
    `5. Check for missing critical drivers. Use AskUserQuestion to fill gaps.`,
    `6. Save the updated profile to .IDE_Plans/architect/architect-profile.json. Use the architecture id (not the long name) as selectedArchitecture.`,
    `7. Read .pi/architecture-library/ and select the best architecture.`,
    `8. Write .IDE_Plans/architect/architect-report.json with selectedArchitecture (the architecture id), confidence, missingResources, reasoning, skillProfile, developmentOrder, feasibility, feasibilityReasoning, techStack, atomicFunctions, systemOverview, components, interfaces, dataFlow, dataModel, deployment, qualityAttributeMapping, adrs, and constraints.`,
    ...
    `11. Call the orchestra_finalize_architecture tool to generate .IDE_Plans/architect/architecture.md, .IDE_Plans/architect/adrs/*.md, .pi/agents/<project>-<architecture-id>-<role>.md, and .pi/skills/<project>-<architecture-id>-<stage>/SKILL.md.`,
