...
import { getArtifactPaths, planStageArtifactManifest } from "../core/paths.js";
...
/** Hard stage-order gate for the manual stage commands. The state machine
 *  (STAGE_TRANSITIONS) is the only source of stage truth; a manual command
 *  may only run from the exact stage its target advances from. Out-of-order
 *  invocation is a hard error naming the exact command that must run first
 *  — the approve flow (/senai-approve) is the only way forward. */
export function ensureStage(
	...
	const cmd = command ?? "/senai-status";
	if (state.currentStage === "none") {
		return { ok: false, reason: `No active run. Start with /senai-plan <mission> before running ${cmd}.` };
	}
	const first = NEXT_COMMAND[state.currentStage] ?? "/senai-status";
	return {
		ok: false,
		reason:
			`Out of order: ${cmd} can only run from stage '${required}'. Current stage is '${state.currentStage}'. ` +
			`Run ${first} first to advance the run, or /senai-status to see where it stands.`,
	};
}
...
export function listMissingStageArtifacts(
	...
	if (stage === "plan") {
		// Shared manifest (core/paths.ts): the same 11-entry list doctor audits,
		// so approve and doctor can never disagree. Approve keeps warn-and-confirm.
		return planStageArtifactManifest(cwd, runId)
			.filter((e) => !isNonEmptyFile(e.path))
			.map((e) => e.label);
	}
	if (stage === "implement") {
		if (!dirHasFiles(artifacts.implementDir)) return ["implement/ (no files)"];
		// The Phase-3 verification reports are written by collectImplementSignals
		// at the implement→implemented approval; they must exist afterwards.
...
/** Interactive commands (pickers, list editors, input prompts) cannot run in
 *  headless/print mode: every prompt would auto-cancel silently. Returns true
 *  when a UI is present; otherwise warns and returns false so the caller bails.
 *  Strict `=== false` check so older test mocks without the flag pass through. */
