...
import { collectImplementSignals, formatImplementSignals, signalsBlockAdvance, checkPlanVerificationGate } from "../implement/signals.js";
import {
		...
		handler: async (_args, ctx) => {
			...
			const lockResult = await withRunLock(
				...
				async () => {
					...
					// stage. Hoisted above the artifact check: collection persists the
					// manifest-diff and arch-rules reports into implement/, and the
					// artifact check below verifies them like any other stage artifact.
					...
					// Hard gate: a plan without executable verification steps can never
					// leave the planning stage. No confirm override — fix the plan first.
					if (current === "planning" && fresh.runId) {
						const gate = checkPlanVerificationGate(ctx.cwd, fresh.runId);
						if (gate.blocked) {
							return { kind: "noop" as const, message: gate.reason ?? "Plan verification gate blocked." };
						}
					}
