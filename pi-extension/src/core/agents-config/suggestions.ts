...
// consolidation. Any other role with web tools is a violation — having two
// agents reach the web confuses the orchestra about who is responsible for
// external knowledge. This is the single source of truth for the rule:
// enforced at spawn time by hooks/spawn-guard.ts and audited by
// doctor's checkWebToolLock (which imports these via doctor/_types.ts).
//
// Tool names are stored lowercase to match the `KNOWN_TOOL_NAMES` convention
// (see `isKnownToolName()` in doctor/_helpers.ts which lowercases on lookup).
...
export const ROLE_LABELS: Record<SenaiRole, string> = {
  ...
  "reviewer-architecture": "Reviewer — Architecture / folder structure",
...
export const ROLE_GUIDANCE: Partial<Record<SenaiRole, RoleGuidance>> = {
  ...
  "reviewer-architecture": "recommended",
