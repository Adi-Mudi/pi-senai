...
export function checkConfigFiles(
	...
	if (agentConfigError) {
		...
		items.push({
			...
			details: ["Run /senai-generate-sub-agents (or /chirpi-generate-architect) to create it, or /senai-configure-agents to configure agents manually."],
		});
	}
	...
	for (const s of allSuggestions) {
		...
		items.push({
			...
			details: [
				...
				`Fix: run /senai-configure-agents-files and set the truth document to ${s.path}. If the assignment is intentional, re-classify the document type via /chirpi-configure-architect-inputs.`,
			],
		});
	}
	...
	if (unverifiable.length > 0) {
		items.push({
			...
			details: [
				...
				"Tip: classify the document type via /chirpi-configure-architect-inputs and give the file a meaningful name and top heading to make it verifiable.",
			],
		});
	}
