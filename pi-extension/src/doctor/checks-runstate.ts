...
import { loadArchitectReport } from "@adi-mudi/pi-chirpi";
import { loadArchitectInputsConfig } from "@adi-mudi/pi-chirpi/dist/pi-extension/src/architect/inputs-config.js";
...
import { loadAgentsFilesConfig } from "../core/agents-config/agents-files-config.js";
...
export function checkSetupProgress(cwd: string): DiagnosticSection {
	...
	const steps: Array<{ done: boolean; label: string; command: string }> = [
		...
		{ done: inputsDone, label: "Architect inputs selected", command: "/chirpi-configure-architect-inputs" },
		{ done: architectDone, label: "Architecture generated", command: "/chirpi-generate-architect" },
...
export function checkRunArtifacts(cwd: string): DiagnosticSection {
	...
	// Shared manifest (core/paths.ts): the same 11-entry list the approve
	// command checks, so doctor and approve can never disagree.
	const planArtifacts: Array<[string, string]> = planStageArtifactManifest(cwd, state.runId).map(
		(e) => [e.label, e.path],
	);
	...
	if (rank >= STAGE_RANK.planned) {
		...
		if (missing.length > 0) {
			...
			items.push({ status: "ok", message: "All 11 plan-stage artifacts exist and are non-empty." });
		}
		...
		if (missing.length > 0) {
			items.push({
				status: "info",
				message: `Run '${state.runId}' is in 'planning' with ${missing.length}/11 plan artifacts still missing`,
				details: [
					...missing,
	...
	// Implement-stage verification reports (Phase 3/4): collectImplementSignals
	// writes both at the implement→implemented approval. Missing reports on a
	// run past implementing mean the approve flow was bypassed or predates the
	// reports — warning, not error, so old runs stay green.
	if (rank >= STAGE_RANK.implemented) {
		const reportArtifacts: Array<[string, string]> = [
			["implement/manifest-diff-report.md", artifacts.manifestDiffReport],
			["implement/arch-rules-report.md", artifacts.archRulesReport],
		];
		const missingReports = missingLabels(reportArtifacts);
		if (missingReports.length > 0) {
			items.push({
				status: "warning",
				message: `Implement stage is marked complete but ${missingReports.length} verification report(s) are missing or empty`,
				details: [
					...missingReports,
					"These are written by /senai-approve when leaving 'implementing'. If you advanced with a manual command, re-run the approve flow or collect signals again.",
				],
			});
		} else {
			items.push({ status: "ok", message: "Both implement-stage verification reports exist and are non-empty." });
		}
	}
...
/** Missing sections for a brief, validated against its own mission type's
 *  required list (bugfix briefs must carry the 4 pack sections). */
function missingBriefSections(brief: string): string[] {
	return validateBriefSections(brief, requiredBriefSectionsFor(parseBriefMissionType(brief)));
}
 ...
 *     that all required sections for the brief's own mission type are
 *     present, in order (base 6 plus pack sections — a bugfix brief must
 *     carry its 4 extra sections). Missing sections → warning naming the
 *     file and the missing section.
