 ...
 *   9. weakened-assertion — skipped/disabled test, commented-out or
 *      tautological assertion (deterministic NEVER-list backing)
 *  10. swallowed-error   — empty catch / catch-and-return-null
 *  11. deleted-test-file — planned test file missing on disk (standalone
 *      detector — a deleted file has no content to scan)
 *
 ...
 *
 * Bugfix runs escalate weakened-assertion / deleted-test-file to blocking
 * (and swallowed-error when the file is the fix target) — that gating lives
 * in signals.ts, which knows the mission type; severities here are the
 * non-bugfix defaults.
 */
...
const HEURISTIC_SEVERITY: Record<HeuristicId, FindingSeverity> = {
    ...
    "god-test": "actionable",
    "deleted-test-file": "actionable",
    "weakened-assertion": "actionable",
    "swallowed-error": "actionable",
};
...
/**
 * Weakened-assertion patterns (diff-level proxies — we have no diff, so we
 * flag the states a "weakened" test ends in): skipped/disabled tests,
 * commented-out assertions, and tautological assertions that always pass.
 * Case-sensitive on purpose: an `// Assert` AAA comment must not match.
 */
const WEAKENED_ASSERTION_PATTERNS: RegExp[] = [
    /\b(?:it|test|describe|context|specify)\.skip\s*\(/,
    /\bx(?:it|test|describe)\s*\(/,
    /^\s*(?:\/\/|#)\s*(?:assert|expect|should)[\w.]*\s*\(/,
    /\bassert(?:\.ok)?\s*\(\s*true\s*[,)]/,
    /\bexpect\s*\(\s*true\s*\)\s*\.\s*to(?:Be|Equal)\s*\(\s*true\s*\)/,
];

/**
 * Swallowed-error patterns: an empty catch block (a lone comment does not
 * count as handling) or a catch whose only exit is `return null/undefined`.
 */
const SWALLOWED_ERROR_PATTERNS: RegExp[] = [
    /catch\s*(?:\([^)]*\))?\s*\{\s*(?:\/\/[^\n]*)?\s*\}/,
    /catch\s*(?:\([^)]*\))?\s*\{[^{}]*?return\s+(?:null|undefined)\s*;?\s*\}/,
];
...
/** Line at a 1-based line number, for excerpts. */
function lineAt(content: string, line: number): string {
    return content.split(/\r?\n/)[line - 1] ?? "";
}

export function detectWeakenedAssertion(content: string, filePath: string): Finding[] {
    const findings: Finding[] = [];
    const lines = content.split(/\r?\n/);
    for (const pattern of WEAKENED_ASSERTION_PATTERNS) {
        for (let i = 0; i < lines.length; i++) {
            const m = lines[i].match(pattern);
            if (m && m[0]) {
                findings.push({
                    heuristic: "weakened-assertion",
                    severity: HEURISTIC_SEVERITY["weakened-assertion"],
                    file: filePath,
                    line: i + 1,
                    column: m.index ?? 0,
                    endLine: i + 1,
                    endColumn: (m.index ?? 0) + m[0].length,
                    message: `Test file weakens or disables a check: '${m[0].trim()}'. Never weaken assertions or skip tests to make the suite green.`,
                    excerpt: trimExcerpt(lines[i]),
                });
...
export function detectSwallowedError(content: string, filePath: string): Finding[] {
    const findings: Finding[] = [];
    for (const pattern of SWALLOWED_ERROR_PATTERNS) {
        const m = content.match(pattern);
        if (m && m.index !== undefined) {
            const line = lineOfOffset(content, m.index);
            findings.push({
                heuristic: "swallowed-error",
                severity: HEURISTIC_SEVERITY["swallowed-error"],
                file: filePath,
                line,
                column: m.index - (content.lastIndexOf("\n", m.index - 1) + 1),
                endLine: line + m[0].split("\n").length - 1,
                endColumn: 0,
                message: "Catch block swallows the error (empty catch or catch-and-return-null). Fix the root cause or rethrow.",
                excerpt: trimExcerpt(lineAt(content, line)),
            });
        }
    }
    return findings;
...
/** True for paths that look like a test file (test dir or .test/.spec suffix). */
export function isTestFilePath(p: string): boolean {
    const norm = p.replace(/\\/g, "/");
    return /(^|\/)(tests?|__tests__|spec)\//.test(norm) || /\.(test|spec)\.[^./]+$/.test(norm);
}

/** Standalone detector: a planned test file (from the plan's `## Files`
 *  manifest) that does not exist on disk. A deleted file has no content to
 *  scan, so this runs against the manifest + disk instead of file bodies. */
export function detectDeletedTestFiles(plannedTestPaths: readonly string[], cwd: string): Finding[] {
    const findings: Finding[] = [];
    for (const p of plannedTestPaths) {
        if (!fs.existsSync(node_path.join(cwd, p))) {
            findings.push({
                heuristic: "deleted-test-file",
                severity: HEURISTIC_SEVERITY["deleted-test-file"],
                file: p,
                line: 0,
                column: 0,
                endLine: 0,
                endColumn: 0,
                message: `Planned test file '${p}' is missing on disk. Never delete a failing test to make the suite green.`,
                excerpt: "",
            });
        }
    }
...
export function scanTestFiles(
    ...
    for (const file of files) {
        const findings = [
            ...
            ...detectWeakenedAssertion(file.content, file.path),
            ...detectSwallowedError(file.content, file.path),
        ];
