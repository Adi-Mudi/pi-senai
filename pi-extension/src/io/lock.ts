...
export function acquireLock(opts: AcquireOptions): AcquireResult {
  ...
  while (true) {
    ...
    // True sleep without burning CPU. This synchronous path remains only for
    // direct acquireLock callers (tests, lock-child helper); withRunLock
    // passes timeoutMs: 0 and waits asynchronously instead, so pi's event
    // loop is never blocked on the production path.
...
export async function withRunLock<T>(
  ...
  // eslint-disable-next-line no-constant-condition
  while (true) {
    // Single-attempt acquire (timeoutMs: 0): this async loop owns the
    // waiting with real sleeps, so the synchronous wait inside acquireLock
    // never blocks pi's event loop on this path.
    const result = acquireLock({ ...opts, timeoutMs: 0, staleMs, heartbeatMs });
    if (result.ok) {
      try {
