...
import { addToGeneratedManifest, loadGeneratedManifest, saveArchitectReport, slugify, writeGeneratedManifest } from "@adi-mudi/pi-chirpi";
...
describe("agent-generator", () => {
  ...
  it("GENERATED_ROLES contains exactly the 15 non-architecture roles", () => {
    ...
    assert.ok(roles.includes("community-researcher"), "community-researcher must be generated");
  });

  it("discussion + community-researcher are the generated roles with WebSearch + FetchURL", () => {
    // Phase 7: community-researcher (web-research) is the brainstorm web
    // specialist. Discussion retains WebSearch for plan-stage consolidation
    // but is no longer the ONLY web-enabled role.
    const WEB_TOOLS = new Set(["WebSearch", "FetchURL"]);
    const withWebTools = GENERATED_ROLES.filter((r) =>
      r.tools.some((t) => WEB_TOOLS.has(t)),
    );
    const roles = withWebTools.map((r) => r.role).sort();
    assert.deepStrictEqual(
      roles,
      ["community-researcher", "discussion"],
      `Web tools: discussion (plan-stage) + community-researcher (brainstorm) are the only web-enabled roles. Found: ${roles.join(", ") || "(none)"}`,
    );
  });
  ...
  it("GENERATOR_VERSION is 10 (framework contract in test-skeleton/full-test mandates)", () => {
    assert.strictEqual(
      GENERATOR_VERSION,
      10,
      `GENERATOR_VERSION must be 10. If you bumped it for another change, update this test and the v10 comment block in generator.ts.`,
    );
  });
...
describe("sub-agent regeneration", () => {
  ...
  it("GENERATOR_VERSION is 10 (framework contract added in v10)", () => {
    assert.strictEqual(GENERATOR_VERSION, 10, "GENERATOR_VERSION must be 10 after the v10 framework-contract mandate addition");
  });

  it("test-skeleton mandate requires integration scaffolds per plan critical path", () => {
    const def = GENERATED_ROLES.find((r) => r.role === "test-skeleton");
    assert.ok(def, "test-skeleton role must exist");
    assert.ok(
      def.mandate.includes("integration test per critical path"),
      "test-skeleton mandate must require one narrow integration test per ## Verification critical path",
    );
  });

  it("full-test mandate forbids silently skipping missing integration tests", () => {
    const def = GENERATED_ROLES.find((r) => r.role === "full-test");
    assert.ok(def, "full-test role must exist");
    assert.ok(
      def.mandate.includes("no integration tests exist"),
      "full-test mandate must require an explicit finding when integration tests are absent",
    );
  });

  it("test-skeleton mandate follows the locked framework's test conventions", () => {
    const def = GENERATED_ROLES.find((r) => r.role === "test-skeleton");
    assert.ok(def, "test-skeleton role must exist");
    assert.ok(
      def.mandate.includes("framework's conventions"),
      "test-skeleton mandate must place/name test files by the locked framework's conventions",
    );
  });

  it("full-test mandate uses the locked framework's standard test runner", () => {
    const def = GENERATED_ROLES.find((r) => r.role === "full-test");
    assert.ok(def, "full-test role must exist");
    assert.ok(
      def.mandate.includes("framework's standard test runner"),
      "full-test mandate must run the locked framework's standard test runner",
    );
  });

  // ─── Phase 7: community-researcher / web-researcher ─────────────────

  it("community-researcher is in GENERATED_ROLES with the brainstorm web tools", () => {
    const def = GENERATED_ROLES.find((r) => r.role === "community-researcher");
    assert.ok(def, "community-researcher must be generated");
    assert.ok(def.tools.includes("WebSearch"));
    assert.ok(def.tools.includes("FetchURL"));
    assert.ok(def.tools.includes("Read"));
    // Must NOT carry Write/Edit/Bash — Phase 7 web tool lock.
    assert.ok(!def.tools.includes("Write"));
    assert.ok(!def.tools.includes("Edit"));
    assert.ok(!def.tools.includes("Bash"));
  });

  it("community-researcher carries bodyFile pointing at the canonical web-researcher-body.md", () => {
    const def = GENERATED_ROLES.find((r) => r.role === "community-researcher");
    assert.ok(def);
    assert.strictEqual(def.bodyFile, "web-researcher-body.md");
  });

  it("buildGeneratedAgentMarkdown renders community-researcher with the canonical body content", () => {
    const def = GENERATED_ROLES.find((r) => r.role === "community-researcher");
    assert.ok(def);
    const out = buildGeneratedAgentMarkdown(
      def,
      "myproject-community-researcher",
      "myproject",
      [],
      null,
    );
    // Standard frontmatter from the generator.
    assert.ok(out.includes("name: myproject-community-researcher"));
    assert.ok(out.includes("tools: WebSearch, FetchURL, Read"));
    assert.ok(out.includes("session-mode: lineage-only"));
    // Canonical body content (Phase 7 contract).
    assert.ok(out.includes("Web Research — Community Researcher"));
    assert.ok(out.toLowerCase().includes("read-only"));
    assert.ok(out.includes("Hard rules"));
    // Footer carries the canonical body file name so doctor can identify it.
    assert.ok(out.includes("web-researcher-body.md"));
  });

  it("loadCanonicalBody returns the body content with frontmatter stripped", () => {
    const body = loadCanonicalBody("web-researcher-body.md");
    assert.ok(body, "expected to load the canonical body");
    assert.ok(!body!.startsWith("---"), "frontmatter should be stripped");
    assert.ok(body!.includes("Web Research — Community Researcher"));
    assert.ok(body!.includes("Hard rules"));
  });

  it("loadCanonicalBody returns null for a missing body file", () => {
    const body = loadCanonicalBody("nonexistent-body.md");
    assert.strictEqual(body, null);
  });

  it("writeGeneratedAgents creates web-researcher.md with the canonical body", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-senai-gen-web-"));
    const def = GENERATED_ROLES.find((r) => r.role === "community-researcher");
    assert.ok(def);
    ...
    assert.ok(created.endsWith("community-researcher.md"));
    const content = fs.readFileSync(path.join(tmpDir, created), "utf8");
    // The agent file name is `${slug}-${role}` where slug = project folder name
    // (slugify lowercases). path.basename strips the tempdir prefix.
    const slug = path.basename(tmpDir).toLowerCase();
    assert.ok(
      content.includes(`name: ${slug}-community-researcher`),
      `expected 'name: ${slug}-community-researcher' in ${created}, got: ${content.slice(0, 200)}`,
    );
    assert.ok(content.includes("Web Research — Community Researcher"));
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
});
