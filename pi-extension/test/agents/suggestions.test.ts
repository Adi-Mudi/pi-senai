...
import type { DiscoveredAgent } from "../../src/core/agents-config/discovery.js";
...
describe("reviewer-architecture registration", () => {
  it("is a Senai role listed right after reviewer-tests", () => {
    const idx = SENAI_ROLES.indexOf("reviewer-architecture");
    assert.ok(idx > 0, "reviewer-architecture must be registered");
    assert.strictEqual(SENAI_ROLES[idx - 1], "reviewer-tests");
  });

  it("defaults to the built-in reviewer agent and has a label", () => {
    assert.strictEqual(DEFAULT_AGENTS["reviewer-architecture"], "reviewer");
    assert.strictEqual(ROLE_LABELS["reviewer-architecture"], "Reviewer — Architecture / folder structure");
  });

  it("is a document role with recommended guidance, visible in the picker", () => {
    assert.ok(DOCUMENT_ROLES.includes("reviewer-architecture"));
    assert.strictEqual(ROLE_GUIDANCE["reviewer-architecture"], "recommended");
    assert.ok(PICKER_ROLES.includes("reviewer-architecture"));
  });
});
