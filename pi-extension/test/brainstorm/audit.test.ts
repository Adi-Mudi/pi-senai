...
describe("audit — countBriefSections", () => {
	...
	it("counts a bugfix brief against its 10 sections (base 6 + 4 pack sections)", () => {
		const brief = [
			"## Problem statement",
			"the retry path deadlocks under load",
			"",
			...
			"",
			"## Reproduction steps",
			"1. two clients, shared key, 100 retries",
			"",
			"## Expected vs actual",
			"expected: completes; actual: hangs",
			"",
			"## Root cause",
			"_TBD_",
			"",
			"## Regression test plan",
			"new test in lock.test.ts",
		].join("\n");
		const c = countBriefSections(brief, "bugfix");
		assert.deepStrictEqual(c, { filled: 9, total: 10 });
	});

	it("non-bugfix types keep the base-6 total even when pack sections exist", () => {
		const brief = "## Problem statement\nx\n\n## Reproduction steps\ny\n";
		const c = countBriefSections(brief, "feature");
		assert.deepStrictEqual(c, { filled: 1, total: 6 });
	});
});
