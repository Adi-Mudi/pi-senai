...
describe("dispatcher — scan types", () => {
	it("community scan dispatches get the 90s timeout", () => {
		const result = prepareDispatch(
			{ agent: "web-research", task: "research official docs", scanType: "community" },
			BRAINSTORM_RUN_ID,
			0,
		);
		...
		if (result.ok) {
			assert.strictEqual(
				result.prepared.subagentArgs.timeoutMs,
				BRAINSTORM_COMMUNITY_DISPATCH_TIMEOUT_MS,
			);
			assert.ok(result.prepared.subagentArgs.timeoutMs >= 90_000);
		}
	});

	it("code and doc scan dispatches keep the 30s timeout", () => {
		// code: generic "scout" resolves to scout-2 (serves code).
		// doc: "scout-4" (PRD/docs audit) serves doc scans.
		const cases = [
			{ scanType: "code" as const, agent: "scout" },
			{ scanType: "doc" as const, agent: "scout-4" },
		];
		for (const { scanType, agent } of cases) {
			const result = prepareDispatch(
				{ agent, task: "x", scanType },
				BRAINSTORM_RUN_ID,
				0,
			);
			assert.strictEqual(result.ok, true, `${scanType} scan should be accepted`);
			if (result.ok) {
				assert.strictEqual(result.prepared.subagentArgs.timeoutMs, BRAINSTORM_DISPATCH_TIMEOUT_MS);
			}
		}
	});

	it("per-type cap refuses the 3rd dispatch of the same scan type", () => {
		const result = prepareDispatch(
			{ agent: "web-research", task: "x", scanType: "community" },
			BRAINSTORM_RUN_ID,
			0,
			SCAN_TYPE_DISPATCH_CAP,
		);
		assert.strictEqual(result.ok, false);
		if (!result.ok) assert.ok(result.reason.includes("cap"));
	});

	it("rejects an agent whose role does not serve the scan type", () => {
		// scout resolves to scout-2 (code); it must not serve community scans.
		const result = prepareDispatch(
			{ agent: "scout", task: "x", scanType: "community" },
			BRAINSTORM_RUN_ID,
			0,
		);
		assert.strictEqual(result.ok, false);
		if (!result.ok) assert.ok(result.reason.includes("community"));
	});

	it("planner is not a scanner for any scan type", () => {
		for (const scanType of ["code", "doc", "community"] as const) {
			const result = prepareDispatch(
				{ agent: "planner", task: "x", scanType },
				BRAINSTORM_RUN_ID,
				0,
			);
			assert.strictEqual(result.ok, false, `planner must not serve ${scanType}`);
		}
	});

	it("dispatches without a scanType keep legacy behavior", () => {
		const result = prepareDispatch({ agent: "scout", task: "x" }, BRAINSTORM_RUN_ID, 0);
		assert.strictEqual(result.ok, true);
		if (result.ok) {
			assert.strictEqual(result.prepared.subagentArgs.timeoutMs, BRAINSTORM_DISPATCH_TIMEOUT_MS);
		}
