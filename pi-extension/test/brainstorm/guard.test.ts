...
describe("guard — brief content per mission type", () => {
	/** validBrief() plus the 4 bugfix pack sections, filled. */
	function validBugfixBrief(): string {
		return [
			validBrief(),
			"",
			"## Reproduction steps",
			"1. Start two clients sharing key k.",
			"2. Fire 100 concurrent retries.",
			"",
			"## Expected vs actual",
			"Expected: no deadlock. Actual: both clients hang after ~30s.",
			"",
			"## Root cause",
			"The retry loop re-acquires the key lock without backoff.",
			"",
			"## Regression test plan",
			"New test: two clients, shared key, 100 retries — must complete.",
		].join("\n");
	}

	it("hard rejects a bugfix brief missing ## Reproduction steps", () => {
		const result = guardBriefContent(validBrief(), "bugfix");
		assert.strictEqual(result.ok, false);
		assert.ok(result.reason?.includes("## Reproduction steps"), result.reason);
	});

	it("hard rejects a bugfix brief with a _TBD_ pack section", () => {
		const brief = validBugfixBrief().replace(
			"The retry loop re-acquires the key lock without backoff.",
			"_TBD_",
		);
		const result = guardBriefContent(brief, "bugfix");
		assert.strictEqual(result.ok, false);
		assert.ok(result.reason?.includes("## Root cause"), result.reason);
	});

	it("accepts a fully filled bugfix brief", () => {
		const result = guardBriefContent(validBugfixBrief(), "bugfix");
		assert.strictEqual(result.ok, true, result.reason);
	});

	it("feature briefs keep the base-6 behavior (backward compat)", () => {
		const result = guardBriefContent(validBrief(), "feature");
		assert.strictEqual(result.ok, true, result.reason);
	});
});
...
describe("guard — approve readiness (new-flow lock)", () => {
	const base: SenaiState = {
		version: 1,
		mission: "",
		runId: "",
		currentStage: "none",
	...
	it("legacy session (no missionType) is exempt and passes", () => {
		const result = guardApproveReadiness(base);
		assert.strictEqual(result.ok, true);
	});

	it("blocks when understanding is not confirmed", () => {
		const result = guardApproveReadiness({ ...base, missionType: "bugfix" });
		assert.strictEqual(result.ok, false);
		assert.ok(result.reason?.includes("not confirmed"));
	});

	it("blocks when a question is still discussing", () => {
		const result = guardApproveReadiness({
			...base,
			missionType: "feature",
			understandingConfirmed: true,
			brainstormQuestions: [
				{ id: "Q1", text: "Scope?", state: "agreed" },
				{ id: "Q2", text: "Auth method?", state: "discussing" },
			],
		});
		assert.strictEqual(result.ok, false);
		assert.ok(result.reason?.includes("Q2"));
	});

	it("blocks when a question is still draft", () => {
		const result = guardApproveReadiness({
			...base,
			missionType: "feature",
			understandingConfirmed: true,
			brainstormQuestions: [{ id: "Q1", text: "Scope?", state: "draft" }],
		});
		assert.strictEqual(result.ok, false);
	});

	it("passes when confirmed and all questions are terminal", () => {
		const result = guardApproveReadiness({
			...base,
			missionType: "upgrade",
			understandingConfirmed: true,
			brainstormQuestions: [
				{ id: "Q1", text: "Scope?", state: "agreed" },
				{ id: "Q2", text: "Drop v1?", state: "not-wanted", reason: "still used" },
				{ id: "Q3", text: "Old approach", state: "replaced", reason: "see Q1" },
			],
		});
		assert.strictEqual(result.ok, true);
	});

	it("passes when confirmed and there are no questions at all", () => {
		const result = guardApproveReadiness({
			...base,
			missionType: "bugfix",
			understandingConfirmed: true,
