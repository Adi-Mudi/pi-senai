...
// The architect commands (senai-suggest-architect / senai-generate-architect /
// senai-configure-architect-inputs) moved to @adi-mudi/pi-chirpi; their tests
// moved with them. Architect state helpers come from the chirpi package.
import { saveArchitectInputsConfig } from "@adi-mudi/pi-chirpi/dist/pi-extension/src/architect/inputs-config.js";
import { saveArchitectReport, slugify } from "@adi-mudi/pi-chirpi";
import { loadState, startRun, advanceStage, defaultState, saveState } from "../src/core/state.js";
import type { SenaiState } from "../src/core/state.js";
import type { Stage } from "../src/core/paths.js";
import { planStageArtifactManifest } from "../src/core/paths.js";
import { checkRunArtifacts } from "../src/doctor/checks-runstate.js";
...
describe("commands", () => {
  ...
  function writePlanArtifacts(cwd: string, runId: string): void {
    ...
    fs.writeFileSync(path.join(runDir, "plan", "plan.md"), "# plan\n\n## Files\n\n| path | layer | action | reason |\n| --- | --- | --- | --- |\n| src/index.ts | core | edit | implement mission |\n\n## Verification\n\n- echo ok\n", "utf8");
    fs.writeFileSync(path.join(runDir, "plan", "plan-overview.md"), "overview\n", "utf8");
    fs.writeFileSync(path.join(runDir, "plan", "discussion-notes.md"), "notes\n", "utf8");
    for (let i = 1; i <= 4; i++) {
      fs.writeFileSync(path.join(runDir, "plan", "scouts", `scout-angle_${i}.md`), "scout\n", "utf8");
    }
    for (const review of ["correctness", "security", "tests", "architecture"]) {
      fs.writeFileSync(path.join(runDir, "plan", "reviews", `review-${review}.md`), "review\n", "utf8");
    }
  }
  ...
  it("senai-approve advances on confirmed missing artifacts and records the outcome", async () => {
    registerCommands(makeApi());
    await commandHandlers["senai-plan"]("Mission", makeCtx());
    // plan.md exists (satisfies the verification gate); scout reports stay missing.
    const runId = loadState(tmpDir).runId;
    const planDir = path.join(tmpDir, ".IDE_Plans", "pi-senai", "runs", runId, "plan");
    ...
    const state = loadState(tmpDir);
    assert.strictEqual(state.currentStage, "implementing");
    assert.match(state.stageResults.planning ?? "", /artifacts: missing 10 \(user confirmed\)$/);
  });
  ...
  it("listMissingStageArtifacts reports per-stage expectations", async () => {
    ...
    // plan: the shared 11-artifact manifest (plan, overview, discussion
    // notes, 4 scouts, 4 reviews) — same list doctor audits.
    assert.strictEqual(listMissingStageArtifacts(tmpDir, runId, "plan").length, 11);
    writePlanArtifacts(tmpDir, runId);
    assert.deepStrictEqual(listMissingStageArtifacts(tmpDir, runId, "plan"), []);

    // implement: any file satisfies the directory check; the two Phase-4
    // verification reports are then required on top.
    ...
    assert.deepStrictEqual(listMissingStageArtifacts(tmpDir, runId, "implement"), [
      "implement/manifest-diff-report.md",
      "implement/arch-rules-report.md",
    ]);
    fs.writeFileSync(path.join(runDir, "implement", "manifest-diff-report.md"), "ok\n", "utf8");
    fs.writeFileSync(path.join(runDir, "implement", "arch-rules-report.md"), "ok\n", "utf8");
    assert.deepStrictEqual(listMissingStageArtifacts(tmpDir, runId, "implement"), []);
  ...
  it("senai-implement blocks when plan artifact is missing", async () => {
    registerCommands(makeApi());
    await commandHandlers["senai-plan"]("Mission", makeCtx());
    // Move to 'planned' without artifacts so the stage gate passes and the
    // artifact check is what fires.
    const state = loadState(tmpDir);
  ...
  it("senai-document rejects running from planned stage", async () => {
    ...
    assert.ok(notifications[0].message.includes("can only run from stage 'implemented'"));
  ...
  it("senai-implement rejects from planning stage", async () => {
    ...
    // The state machine gate fires before any artifact check and names the
    // exact command that must run first.
    assert.ok(notifications[0].message.includes("Out of order"));
    assert.ok(notifications[0].message.includes("can only run from stage 'planned'"));
    assert.ok(notifications[0].message.includes("Run /senai-approve first"));
    assert.strictEqual(notifications[0].type, "error");
    assert.strictEqual(loadState(tmpDir).currentStage, "planning");
  });
  ...
  it("senai-deliver rejects running from the implementing stage", async () => {
    registerCommands(makeApi());
    await commandHandlers["senai-plan"]("Mission", makeCtx());
    const planRunId = loadState(tmpDir).runId;
    writePlanArtifacts(tmpDir, planRunId);
    await commandHandlers["senai-approve"]("", makeCtx()); // planning -> planned -> implementing
    ...
    assert.ok(notifications[0].message.includes("can only run from stage 'documented'"));
    assert.strictEqual(notifications[0].type, "error");
    assert.strictEqual(loadState(tmpDir).currentStage, "implementing");
  });
  ...
  it("senai-configure-agents-files shows only the picker-visible roles", async () => {
    ...
    assert.strictEqual(labels.length, 9, "8 picker-visible roles + Finish");
    ...
    assert.ok(labels.some((l) => l.includes("Reviewer — Architecture")), "architecture reviewer is shown");
...
describe("shared plan-artifact manifest (approve + doctor)", () => {
  function makeRun(missingFrom: string[]): { tmpDir: string; runId: string } {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "senai-manifest-"));
    const runId = "2099-01-01-00-00-shared";
    const all = planStageArtifactManifest(tmpDir, runId);
    for (const entry of all) {
      if (missingFrom.includes(entry.label)) continue;
      fs.mkdirSync(path.dirname(entry.path), { recursive: true });
      fs.writeFileSync(entry.path, "content");
    }
    saveState(tmpDir, { ...defaultState(), mission: "m", runId, currentStage: "planned" });
    return { tmpDir, runId };
  }

  it("approve and doctor report identical missing-artifact sets for the same run dir", () => {
    const missing = ["plan/discussion-notes.md", "plan/reviews/review-security.md", "plan/scouts/scout-angle_3.md"];
    const { tmpDir, runId } = makeRun(missing);
    try {
      const approveMissing = listMissingStageArtifacts(tmpDir, runId, "plan");
      const section = checkRunArtifacts(tmpDir);
      const warning = section.items.find((i) => i.status === "warning" && i.message.includes("missing or empty"));
      assert.ok(warning, "doctor must warn about the missing artifacts");
      const doctorMissing = (warning.details ?? []).filter((d) => d.startsWith("plan/"));
      assert.deepStrictEqual([...approveMissing].sort(), [...doctorMissing].sort());
      assert.deepStrictEqual([...approveMissing].sort(), [...missing].sort());
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });

  it("both report zero missing when all 11 plan artifacts exist", () => {
    const { tmpDir, runId } = makeRun([]);
    try {
      assert.deepStrictEqual(listMissingStageArtifacts(tmpDir, runId, "plan"), []);
      const section = checkRunArtifacts(tmpDir);
      assert.ok(section.items.some((i) => i.status === "ok" && i.message.includes("All 11 plan-stage artifacts")));
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
...
describe("coverage audit gaps", () => {
  ...
  function writePlanArtifacts(state: SenaiState): void {
    ...
    fs.writeFileSync(path.join(planDir, "discussion-notes.md"), "# Notes\n");
    for (let i = 1; i <= 4; i++) {
    ...
    for (const review of ["correctness", "security", "tests", "architecture"]) {
      fs.writeFileSync(path.join(reviewsDir, `review-${review}.md`), `# Review ${review}\n`);
    }
  }
  ...
  it("senai-implement warns when plan artifacts exist but the stage is not planned", async () => {
    ...
    await commandHandlers["senai-approve"]("", makeCtx()); // planning -> planned -> implementing
    // artifacts written before the approve above
