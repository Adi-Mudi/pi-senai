...
describe("role expected artifacts (Phase 4 fallback)", () => {
  beforeEach(() => resetCompletionGuard());

  it("verifies the reviewer-architecture artifact even when the task omits the path", () => {
    const cwd = makeTmp();
    setupActiveRun(cwd); // planning, runId r1
    saveAgentConfig(cwd, {
      version: 1,
      agents: { "reviewer-architecture": "proj-arch-reviewer-architecture" },
    });
    recordSpawnArtifacts(
      "subagent",
      { name: "ra-1", agent: "proj-arch-reviewer-architecture", task: "Review the plan layout." },
      cwd,
    );
    const warning = completionWarning('Sub-agent "ra-1" completed (30s).', cwd);
    assert.ok(warning, "missing role artifact must warn even without a task path");
    assert.ok(warning.includes("review-architecture.md"), "names the role artifact");
    assert.ok(warning.includes("subagent_resume"), "keeps the auto-resume pattern");
  });

  it("stays silent once the role artifact exists", () => {
    const cwd = makeTmp();
    setupActiveRun(cwd);
    saveAgentConfig(cwd, {
      version: 1,
    ...
    const written = path.join(cwd, ".IDE_Plans/pi-senai/runs/r1/plan/reviews/review-architecture.md");
    fs.mkdirSync(path.dirname(written), { recursive: true });
    fs.writeFileSync(written, "# review\n", "utf8");
    assert.strictEqual(completionWarning('Sub-agent "ra-1" completed (30s).', cwd), undefined);
  });

  it("does not add role artifacts outside the planning stage", () => {
    const cwd = makeTmp();
    saveState(cwd, { ...defaultState(), currentStage: "implementing", runId: "r1" });
    saveAgentConfig(cwd, {
      version: 1,
      agents: { "reviewer-architecture": "proj-arch-reviewer-architecture" },
    });
  ...
  it("ignores agents not mapped to a reviewer role", () => {
    const cwd = makeTmp();
    setupActiveRun(cwd);
    saveAgentConfig(cwd, { version: 1, agents: { planner: "proj-arch-planner" } });
    recordSpawnArtifacts(
      "subagent",
      { name: "p-1", agent: "proj-arch-planner", task: "Plan the mission." },
      cwd,
    );
    assert.strictEqual(completionWarning('Sub-agent "p-1" completed (30s).', cwd), undefined);
  });
});
