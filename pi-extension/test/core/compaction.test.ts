...
describe("compaction", () => {
  ...
  it("includes the brainstorm document ID + path while a brainstorm is in flight", () => {
    const tmpDir = makeTmpDir("pi-senai-compaction-brainstorm-");
    // Brainstorm in flight, no run yet (stage stays "none").
    let state = startBrainstorm(tmpDir, "login broken", loadState(tmpDir));
    state = confirmUnderstanding(tmpDir, state, "bugfix");
    syncDecisionsToDiscussionDoc(tmpDir, state.brainstormRunId!, [
      { id: "Q1", text: "Where does it fail?", state: "agreed" },
    ]);

    const summary = buildSenaiCompactionSummary(tmpDir);

    assert.ok(summary, "brainstorm in flight must still produce a summary");
    assert.ok(summary.includes(`Brainstorm run ID: ${state.brainstormRunId}`));
    assert.ok(summary.includes("Understanding confirmed: yes"));
    assert.ok(summary.includes("Mission type: bugfix"));
    assert.ok(summary.includes("mission-brief.md"));
    assert.ok(summary.includes("discussion-01-decisions.md"), "document path must be in the summary");
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
