...
describe("mission-brief — decision ledger", () => {
  let cwd: string;
  beforeEach(() => {
    cwd = makeTmp();
  });

  const brainstormRunId = "2026-09-11-13-00-brainstorm-test";

  it("renderDecisionsBlock renders Agreed / Not wanted / Open with reasons", () => {
    const block = renderDecisionsBlock([
      { id: "Q1", text: "Use JWT?", suggestedAnswer: "yes", state: "agreed" },
      { id: "Q2", text: "Drop sessions?", state: "not-wanted", reason: "still needed" },
      { id: "Q3", text: "Old idea", state: "replaced", reason: "see Q1" },
      { id: "Q4", text: "Token TTL?", state: "discussing" },
    ]);
    assert.ok(block.includes("## Agreed"));
    assert.ok(block.includes("Q1: Use JWT? — yes"));
    assert.ok(block.includes("## Not wanted"));
    assert.ok(block.includes("Q2: Drop sessions? — reason: still needed"));
    assert.ok(block.includes("~~Old idea~~ (superseded)"));
    assert.ok(block.includes("## Open"));
    assert.ok(block.includes("Q4: Token TTL? (state: discussing)"));
  });

  it("syncDecisionsToDiscussionDoc creates the doc, then refreshes the block in place", () => {
    const first = syncDecisionsToDiscussionDoc(cwd, brainstormRunId, [
      { id: "Q1", text: "Scope?", state: "draft" },
    ]);
    assert.ok(fs.existsSync(first));
    let content = fs.readFileSync(first, "utf8");
    assert.ok(content.includes(DECISIONS_BLOCK_START));
    assert.ok(content.includes("Q1: Scope? (state: draft)"));

    const second = syncDecisionsToDiscussionDoc(cwd, brainstormRunId, [
      { id: "Q1", text: "Scope?", state: "agreed" },
    ]);
    assert.strictEqual(second, first, "same doc is updated, not duplicated");
    content = fs.readFileSync(first, "utf8");
    assert.ok(content.includes("Q1: Scope?"));
    assert.ok(!content.includes("(state: draft)"), "old open state must be gone");
    assert.strictEqual(
      content.split(DECISIONS_BLOCK_START).length - 1,
      1,
      "exactly one decisions block",
    );
  });

  it("appendOutOfScopeDecision adds the rejection with reason, once", () => {
    // Seed a real brainstorm brief via recordDiscussion.
    const loc = recordDiscussion({
      cwd,
      brainstormRunId,
      label: "decisions",
    ...
    const question = { id: "Q2", text: "Drop v1 API?", state: "not-wanted" as const, reason: "clients still use it" };

    const added = appendOutOfScopeDecision(loc.briefPath, question);
    assert.strictEqual(added, true);
    const brief = fs.readFileSync(loc.briefPath, "utf8");
    const scopeIdx = brief.indexOf("## Out-of-scope");
    const bulletIdx = brief.indexOf("Q2: Drop v1 API? — not wanted: clients still use it");
    assert.ok(scopeIdx !== -1 && bulletIdx > scopeIdx, "bullet must land inside Out-of-scope");

    // Idempotent: second call does not duplicate.
    const again = appendOutOfScopeDecision(loc.briefPath, question);
    assert.strictEqual(again, false);
    const briefAfter = fs.readFileSync(loc.briefPath, "utf8");
    assert.strictEqual(briefAfter.split("- Q2:").length - 1, 1);
  });
});