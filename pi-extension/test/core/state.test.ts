...
describe("state — new brainstorm flow fields", () => {
  let tmpDir: string;
  ...
  it("confirmUnderstanding sets the lock flag and mission type, and they survive save/load", () => {
    const started = startBrainstorm(tmpDir, "login broken", defaultState());
    const confirmed = confirmUnderstanding(tmpDir, started, "bugfix");
    assert.strictEqual(confirmed.understandingConfirmed, true);
    assert.strictEqual(confirmed.missionType, "bugfix");

    const loaded = loadState(tmpDir);
    assert.strictEqual(loaded.understandingConfirmed, true);
    assert.strictEqual(loaded.missionType, "bugfix");
  });

  it("setScansSelected persists the scan selection", () => {
    const started = startBrainstorm(tmpDir, "add feature", defaultState());
    setScansSelected(tmpDir, started, ["code", "community"]);
    const loaded = loadState(tmpDir);
    assert.deepStrictEqual(loaded.scansSelected, ["code", "community"]);
  });

  it("upsertBrainstormQuestion adds, then updates by id", () => {
    let state = startBrainstorm(tmpDir, "add feature", defaultState());
    state = upsertBrainstormQuestion(tmpDir, state, {
      id: "Q1",
      text: "Scope?",
      suggestedAnswer: "header only",
      state: "draft",
    });
    state = upsertBrainstormQuestion(tmpDir, state, {
      id: "Q1",
    ...
    const loaded = loadState(tmpDir);
    assert.strictEqual(loaded.brainstormQuestions?.length, 1, "update must not duplicate the id");
    assert.strictEqual(loaded.brainstormQuestions?.[0].state, "not-wanted");
    assert.strictEqual(loaded.brainstormQuestions?.[0].reason, "user rejected");
  });

  it("a fresh brainstorm resets the new-flow fields", () => {
    let state = startBrainstorm(tmpDir, "first", defaultState());
    state = confirmUnderstanding(tmpDir, state, "feature");
    state = upsertBrainstormQuestion(tmpDir, state, { id: "Q1", text: "t", state: "agreed" });
    state = setScansSelected(tmpDir, state, ["doc"]);

    // Simulate a new brainstorm by clearing the id, then starting again.
    const cleared = { ...loadState(tmpDir), brainstormRunId: undefined };
    const fresh = startBrainstorm(tmpDir, "second", cleared);
    assert.strictEqual(fresh.understandingConfirmed, undefined);
    assert.strictEqual(fresh.missionType, undefined);
    assert.strictEqual(fresh.scansSelected, undefined);
    assert.strictEqual(fresh.brainstormQuestions, undefined);
    assert.strictEqual(fresh.brainstormDispatchCount, 0);
  });

  it("confirmUnderstanding accepts all 6 mission types", () => {
    for (const type of ["bugfix", "feature", "upgrade", "explore", "docs", "test"] as const) {
      const started = startBrainstorm(tmpDir, `mission ${type}`, defaultState());
      const confirmed = confirmUnderstanding(tmpDir, started, type);
      assert.strictEqual(confirmed.missionType, type);
      assert.strictEqual(loadState(tmpDir).missionType, type);
    }
  });

  it("migration keeps the new mission types and drops garbage", () => {
    const statePath = path.join(tmpDir, ".IDE_Plans", "pi-senai", "state.json");
    for (const type of ["explore", "docs", "test"]) {
      fs.mkdirSync(path.dirname(statePath), { recursive: true });
      fs.writeFileSync(statePath, JSON.stringify({ version: 0, missionType: type }));
      assert.strictEqual(loadState(tmpDir).missionType, type, `migration must keep ${type}`);
    }
    fs.writeFileSync(statePath, JSON.stringify({ version: 0, missionType: "banana" }));
    assert.strictEqual(loadState(tmpDir).missionType, undefined, "garbage must be dropped");
  });
});
