...
import { saveArchitectInputsConfig } from "@adi-mudi/pi-chirpi/dist/pi-extension/src/architect/inputs-config.js";
import { saveArchitectProfile, saveArchitectReport, writeGeneratedManifest } from "@adi-mudi/pi-chirpi";
import { getProjectSlug, GENERATOR_VERSION } from "../../src/agents/generator.js";
import { createEmptyDrivers, saveDrivers } from "@adi-mudi/pi-chirpi/dist/pi-extension/src/architect/drivers.js";
...
describe("doctor architecture validation", () => {
  ...
  const GENERATED_ROLES = ["planner", "implementer", "reviewer-correctness", "reviewer-security", "reviewer-tests", "reviewer-architecture"];
  ...
  // v11 body: no framework contract lines. Used to simulate stale agents.
  const V11_BODY =
    "Read `.pi/architect/architecture.md` and the relevant ADRs in `.pi/architect/adrs/` before acting.\n\n## Forbidden patterns\n\n- global mutable state";
  // v12 adds the framework contract: planner demands the plan's `## Framework`
  // field, reviewer-architecture checks the locked framework's layout map and
  // `## Rules`, implementer carries the framework lock line.
  const V12_FRAMEWORK_BLOCK =
    "\n\n## Plan contract\n\nEvery plan must carry a `## Framework` section naming the locked framework.\n\n6. When the plan locks a framework, every planned file matches that framework's layout map.\n7. No planned file violates the framework map's `## Rules`.\n\nFramework lock: follow the locked framework's layout map and never break its rules.";
  ...
  function writeGeneratedAgent(
    ...
    const body = overrides?.body ?? `${V11_BODY}${V12_FRAMEWORK_BLOCK}`;
    const content = [
      "---",
      `name: ${name}`,
  ...
  it("mapping check passes when all eight roles point to the generated agents", () => {
    ...
    saveAgentConfig(tmpDir, {
      ...
      agents: {
        ...
        "reviewer-architecture": `${SLUG}-${ARCH}-reviewer-architecture`,
    ...
    assert.strictEqual(section.items.filter((i) => i.status === "ok").length, 8);
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
  ...
  it("content check errors on a stale v11 planner body missing the ## Framework requirement", () => {
    const tmpDir = makeTmpDir("doctor-content-v11-planner-");
    saveTestProfile(tmpDir);
    writeGeneratedSkills(tmpDir);
    writeGeneratedAgent(tmpDir, "planner", { body: V11_BODY });
    const report = runSenaiDiagnostic(tmpDir);
    const section = findSection(report, "Generated agent content");
    const item = section.items.find((i) => i.message.includes(`${SLUG}-${ARCH}-planner`));
    assert.ok(item);
    assert.strictEqual(item.status, "error");
    assert.ok(item.details?.some((d) => d.includes("## Framework")));
    assert.ok(item.details?.some((d) => d.includes("/chirpi-generate-architect")));
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it("content check errors on a stale v11 reviewer-architecture body missing the framework checklist items", () => {
    const tmpDir = makeTmpDir("doctor-content-v11-revarch-");
    saveTestProfile(tmpDir);
    writeGeneratedSkills(tmpDir);
    writeGeneratedAgent(tmpDir, "reviewer-architecture", { body: V11_BODY });
    const report = runSenaiDiagnostic(tmpDir);
    const section = findSection(report, "Generated agent content");
    const item = section.items.find((i) => i.message.includes(`${SLUG}-${ARCH}-reviewer-architecture`));
    assert.ok(item);
    assert.strictEqual(item.status, "error");
    assert.ok(item.details?.some((d) => d.includes("framework review checklist")));
    assert.ok(item.details?.some((d) => d.includes("/chirpi-generate-architect")));
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it("content check errors on a stale v11 implementer body missing the framework lock line", () => {
    const tmpDir = makeTmpDir("doctor-content-v11-impl-");
    saveTestProfile(tmpDir);
    writeGeneratedSkills(tmpDir);
    writeGeneratedAgent(tmpDir, "implementer", { body: V11_BODY });
    const report = runSenaiDiagnostic(tmpDir);
    const section = findSection(report, "Generated agent content");
    const item = section.items.find((i) => i.message.includes(`${SLUG}-${ARCH}-implementer`));
    assert.ok(item);
    assert.strictEqual(item.status, "error");
    assert.ok(item.details?.some((d) => d.includes("framework lock line")));
    assert.ok(item.details?.some((d) => d.includes("/chirpi-generate-architect")));
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
  ...
  it("mapping check uses the architecture id from the profile", () => {
    ...
    saveAgentConfig(tmpDir, {
      ...
      agents: {
        ...
        "reviewer-architecture": `${SLUG}-hexagonal-reviewer-architecture`,
  ...
  it("mapping check reports mixed ok and error items for partial mapping", () => {
    ...
    assert.strictEqual(section.items.filter((i) => i.status === "error").length, 6);
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
  ...
  it("drift advice names the right regenerate command per file type", () => {
    ...
    assert.ok(!teamItem?.details?.some((d) => d.includes("chirpi-generate-architect")));
    assert.ok(archItem?.details?.some((d) => d.includes("/chirpi-generate-architect")), "architecture advice names the chirpi command");
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
  ...
  it("resource check warns when the Integration testing subsection is missing", () => {
    const tmpDir = makeTmpDir("doctor-res-nointeg-");
    writeFile(
      tmpDir,
      path.join(".pi", "technologies", "rust.md"),
    ...
    const warning = section.items.find((i) => i.status === "warning" && i.message.includes("rust.md"));
    assert.ok(warning);
    assert.ok(warning.details?.some((d) => d.includes("Integration testing")), "missing Integration testing subsection must be flagged");
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
  ...
  it("architecture setup warns on non-high confidence and counts a non-empty library", () => {
    ...
    // The bundled pi-chirpi library merges with the project library, so the
    // count includes the bundled entries.
    const library = section.items.find((i) => /Architecture library has \d+ entries\./.test(i.message));
    assert.ok(library);
...
describe("doctor setup progress", () => {
  ...
  it("with only files.json the next step is /chirpi-configure-architect-inputs", () => {
    const tmpDir = setupThrough(1);
    const section = findProgress(runSenaiDiagnostic(tmpDir));
    assert.ok(guidanceMessage(section).includes("Next: run /chirpi-configure-architect-inputs"));
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it("with architect inputs done the next step is /chirpi-generate-architect", () => {
    const tmpDir = setupThrough(2);
    const section = findProgress(runSenaiDiagnostic(tmpDir));
    assert.ok(guidanceMessage(section).includes("Next: run /chirpi-generate-architect"));
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
...
describe("coverage audit gaps", () => {
  ...
  it("architecture setup falls back to the bundled pi-chirpi library when the project has none", () => {
    const tmpDir = makeTmpDir("doctor-arch-no-library-");

    const report = runSenaiDiagnostic(tmpDir);
    const section = findGapSection(report, "Architecture setup");
    // pi-chirpi ships a bundled architecture library, so the library is
    // never empty — the project-local .pi/architecture-library/ only
    // overrides bundled entries with the same id.
    const item = section.items.find((i) => /Architecture library has \d+ entries\./.test(i.message));
    assert.ok(item);
    assert.strictEqual(item.status, "ok");
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
  ...
  it("architecture setup reports info when no architect inputs are configured", () => {
    ...
    assert.ok(item);
    assert.strictEqual(item.status, "info");
    assert.ok(item.message.includes("/chirpi-configure-architect-inputs"));
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
...
describe("doctor strictness — collision warning and run artifact audit", () => {
  const PLAN_ARTIFACTS = [
    ...
    "plan/reviews/review-architecture.md",
  ];
  ...
  it("run artifact audit is clean when all plan artifacts exist", () => {
    ...
    assert.ok(
      section?.items.some(
        (i) => i.status === "ok" && i.message.includes("All 11 plan-stage artifacts"),
      ),
  ...
  it("lists all 11 plan artifacts when the run directory does not exist at all", () => {
    const tmpDir = makeTmpDir("doctor-audit-nodir-");
    saveState(tmpDir, { ...defaultState(), currentStage: "implementing", runId: "ghost-run" });
    const report = runSenaiDiagnostic(tmpDir);
    const section = report.sections.find((s) => s.title === "Run artifacts");
    const warning = section?.items.find(
      (i) => i.status === "warning" && i.message.includes("11 artifact(s)"),
    );
...
describe("doctor hard enforcement gates (Phase 4)", () => {
  function findGatesSection(report: ReturnType<typeof runSenaiDiagnostic>) {
    const section = report.sections.find((s) => s.title === "Hard enforcement gates");
    assert.ok(section, "Hard enforcement gates section must exist");
    return section;
  }

  it("reports gates healthy on a valid project with an architecture", () => {
    const tmpDir = makeTmpDir("doctor-gates-ok-");
    saveArchitectProfile(tmpDir, {
      projectName: "Test Project",
      projectSlug: "test-project",
      selectedArchitecture: "modular-monolith",
    ...
    writeFile(
      tmpDir,
      ".pi/agents/test-project-modular-monolith-reviewer-architecture.md",
      "---\nname: test-project-modular-monolith-reviewer-architecture\ndescription: r\n---\n# r",
    );
    const section = findGatesSection(runSenaiDiagnostic(tmpDir));
    assert.strictEqual(
      section.items.filter((i) => i.status === "error").length,
      0,
      `no errors expected; got ${JSON.stringify(section.items.filter((i) => i.status === "error"))}`,
    );
    assert.ok(section.items.some((i) => i.message.includes("Spawn guard armed")), "spawn guard wiring ok");
    assert.ok(section.items.some((i) => i.message.includes("Web tool lock")), "web lock rule listed");
    assert.ok(section.items.some((i) => i.message.includes("Stage machine")), "stage machine sane");
    assert.ok(section.items.some((i) => i.message.includes("reviewer-architecture agent exists")), "reviewer-architecture ok");
    assert.ok(section.items.some((i) => i.message.includes("Layer map")), "layer map armed");
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it("errors when the reviewer-architecture agent is missing", () => {
    const tmpDir = makeTmpDir("doctor-gates-noagent-");
    saveArchitectProfile(tmpDir, {
      projectName: "Test Project",
      projectSlug: "test-project",
      selectedArchitecture: "modular-monolith",
    ...
    const section = findGatesSection(runSenaiDiagnostic(tmpDir));
    const item = section.items.find((i) => i.message.includes("reviewer-architecture agent is missing"));
    assert.ok(item, "missing agent reported");
    assert.strictEqual(item.status, "error");
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it("warns when the cadence state file is corrupt", () => {
    const tmpDir = makeTmpDir("doctor-gates-cadence-");
    writeFile(tmpDir, ".IDE_Plans/pi-senai/spawn-cadence.json", "{ not json");
    const section = findGatesSection(runSenaiDiagnostic(tmpDir));
    const item = section.items.find((i) => i.message.includes("Cadence state file is corrupt"));
    assert.ok(item, "corrupt cadence file reported");
    assert.strictEqual(item.status, "warning");
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it("warns when the cadence state file has an invalid tier", () => {
    const tmpDir = makeTmpDir("doctor-gates-tier-");
    writeFile(tmpDir, ".IDE_Plans/pi-senai/spawn-cadence.json", JSON.stringify({ tier: "Z" }));
    const section = findGatesSection(runSenaiDiagnostic(tmpDir));
    const item = section.items.find((i) => i.message.includes("Cadence state file is corrupt"));
    assert.ok(item, "invalid tier reported");
    assert.strictEqual(item.status, "warning");
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it("skips architecture-dependent checks with info when no profile exists", () => {
    const tmpDir = makeTmpDir("doctor-gates-noprofile-");
    const section = findGatesSection(runSenaiDiagnostic(tmpDir));
    assert.ok(
      section.items.some((i) => i.status === "info" && i.message.includes("No architecture generated yet")),
      "profile-less project is an info skip, not an error",
    );
    assert.strictEqual(section.items.filter((i) => i.status === "error").length, 0);
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it("reports the framework library present/valid and the framework gate wired", () => {
    const tmpDir = makeTmpDir("doctor-gates-framework-");
    const section = findGatesSection(runSenaiDiagnostic(tmpDir));
    const libItem = section.items.find((i) => i.message.includes("Framework library"));
    assert.ok(libItem, "framework library item present");
    assert.strictEqual(libItem.status, "ok", `library must validate; got: ${JSON.stringify(libItem)}`);
    const wireItem = section.items.find((i) => i.message.includes("Framework rules gate armed"));
    assert.ok(wireItem, "framework gate wiring item present");
    assert.strictEqual(wireItem.status, "ok");
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
});

describe("doctor — Framework lock section (chirpi checkFrameworkConfig)", () => {
  it("reports info when no framework lock exists", () => {
    const tmpDir = makeTmpDir("doctor-fwlock-none-");
    const report = runSenaiDiagnostic(tmpDir);
    const section = report.sections.find((s) => s.title === "Framework lock");
    assert.ok(section, "Framework lock section must be present");
    assert.ok(section.items.some((i) => i.status === "info" && i.message.includes("No framework lock configured")));
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it("reports the locked framework with its source", () => {
    const tmpDir = makeTmpDir("doctor-fwlock-ok-");
    writeFile(
      tmpDir,
      ".pi/senai/framework.json",
      JSON.stringify({ id: "express", source: "manual", setAt: new Date().toISOString() }),
    );
    const report = runSenaiDiagnostic(tmpDir);
    const section = report.sections.find((s) => s.title === "Framework lock");
    assert.ok(section, "Framework lock section must be present");
    assert.ok(section.items.some((i) => i.status === "ok" && i.message.includes("Framework locked: express")));
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it("reports a malformed framework.json as an error", () => {
    const tmpDir = makeTmpDir("doctor-fwlock-bad-");
    writeFile(tmpDir, ".pi/senai/framework.json", "{ not json");
    const report = runSenaiDiagnostic(tmpDir);
    const section = report.sections.find((s) => s.title === "Framework lock");
    assert.ok(section, "Framework lock section must be present");
    assert.ok(section.items.some((i) => i.status === "error" && i.message.includes("malformed")));
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it("reports an unknown framework id as an error", () => {
    const tmpDir = makeTmpDir("doctor-fwlock-unknown-");
    writeFile(
      tmpDir,
      ".pi/senai/framework.json",
      JSON.stringify({ id: "ghost-framework", source: "manual", setAt: new Date().toISOString() }),
    );
    const report = runSenaiDiagnostic(tmpDir);
    const section = report.sections.find((s) => s.title === "Framework lock");
    assert.ok(section, "Framework lock section must be present");
    assert.ok(section.items.some((i) => i.status === "error" && i.message.includes("not in the framework library")));
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
});
