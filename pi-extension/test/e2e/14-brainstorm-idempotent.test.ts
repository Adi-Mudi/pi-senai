...
// `/senai-brainstorm-approve` is documented as idempotent: a second call
...
describe("e2e/14-brainstorm-idempotent", () => {
	...
	it("a second /senai-brainstorm-approve on the same brief is a no-op (skips on pi 0.84.3 prompt bug)", { timeout: 60_000 }, async (t) => {
		...
		try {
			...
			// First brainstorm + approve.
			await client.request("prompt", { text: "/senai-brainstorm pick the best stack" });
