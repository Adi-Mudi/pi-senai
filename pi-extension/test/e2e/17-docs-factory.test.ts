...
describe("e2e/17-docs-factory", () => {
	...
	it("generateDocsStructure creates the docs skeleton folders and template stubs", { timeout: 60_000 }, async (t) => {
		...
		// The fixture already ships a README.md, and the docs factory never
		// overwrites an existing doc. Assert the stub marker on CHANGELOG.md,
		// which the factory really did generate.
		const changelogContent = fs.readFileSync(changelog, "utf8");
		assert.ok(changelogContent.includes("pi-senai doc stub"), "CHANGELOG stub should carry the pi-senai doc stub marker");
	});
