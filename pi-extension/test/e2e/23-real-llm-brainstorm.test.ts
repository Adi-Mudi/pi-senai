...
// Tier 2: real /senai-brainstorm draft. The parent LLM should produce
...
describe("e2e/23-real-llm-brainstorm", () => {
	...
	it("real /senai-brainstorm captures at least one AskUserQuestion-style request", { timeout: 120_000 }, async (t) => {
		...
		try {
			...
			try {
				await client.request("prompt", { text: "/senai-brainstorm pick the right database" });
			...
			// /senai-brainstorm in v1 of this feature is parent-LLM-driven,
			...
			assert.ok(hasTranscript, "brainstorm transcript should be written");
		} catch (err) {
