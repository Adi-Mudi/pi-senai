...
export class RpcClient {
	...
	private dispatchLine(line: string): void {
		...
		if (msg && typeof msg.id === "string" && this.pending.has(msg.id)) {
			// Streaming events (e.g. bash_execution_update) share the request id
			// on pi >= 0.85. Only the terminal `response` message settles the
			// pending promise; updates fall through to the event listeners.
