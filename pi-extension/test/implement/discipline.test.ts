...
describe("detectWeakenedAssertion (Phase 4)", () => {
    it("flags a skipped test (it.skip)", () => {
        const content = `it.skip("flaky", () => { assert.equal(1, 1); });`;
        const findings = detectWeakenedAssertion(content, F);
        assert.strictEqual(findings.length, 1);
        assert.strictEqual(findings[0].heuristic, "weakened-assertion");
        assert.strictEqual(findings[0].severity, "actionable");
    });

    it("flags a commented-out assertion", () => {
        const content = `
            it("x", () => {
                // assert.equal(a, b);
            });
    ...
    it("flags a tautological assertion (assert.ok(true))", () => {
        const content = `
            it("x", () => {
                assert.ok(true);
            });
    ...
    it("does NOT flag a real assertion or an `// Assert` AAA comment", () => {
        const content = `
            it("real", () => {
                // Assert
                assert.equal(1 + 1, 2);
...
describe("detectSwallowedError (Phase 4)", () => {
    it("flags an empty catch block", () => {
        const content = `try { risky(); } catch (e) {}`;
        const findings = detectSwallowedError(content, F);
        assert.strictEqual(findings.length, 1);
        assert.strictEqual(findings[0].heuristic, "swallowed-error");
        assert.strictEqual(findings[0].severity, "actionable");
    });

    it("flags a catch whose only exit is return null", () => {
        const content = `try { risky(); } catch (e) { return null; }`;
        const findings = detectSwallowedError(content, F);
        assert.strictEqual(findings.length, 1);
        assert.strictEqual(findings[0].heuristic, "swallowed-error");
    });

    it("does NOT flag a catch that rethrows or logs", () => {
        const content = [
...
describe("detectDeletedTestFiles (Phase 4)", () => {
    it("flags a planned test file that is missing on disk", () => {
        const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "senai-deltest-"));
        try {
            fs.mkdirSync(path.join(cwd, "test"), { recursive: true });
            fs.writeFileSync(path.join(cwd, "test", "a.test.ts"), "export {};\n");
            const findings = detectDeletedTestFiles(["test/a.test.ts", "test/b.test.ts"], cwd);
            assert.strictEqual(findings.length, 1);
            assert.strictEqual(findings[0].heuristic, "deleted-test-file");
            assert.strictEqual(findings[0].file, "test/b.test.ts");
            assert.strictEqual(findings[0].severity, "actionable");
        } finally {
    ...
    it("returns nothing when every planned test file exists", () => {
        const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "senai-deltest-"));
        try {
            fs.mkdirSync(path.join(cwd, "test"), { recursive: true });
            fs.writeFileSync(path.join(cwd, "test", "a.test.ts"), "export {};\n");
            fs.writeFileSync(path.join(cwd, "test", "b.test.ts"), "export {};\n");
            const findings = detectDeletedTestFiles(["test/a.test.ts", "test/b.test.ts"], cwd);
            assert.strictEqual(findings.length, 0);
        } finally {
            fs.rmSync(cwd, { recursive: true, force: true });
...
describe("isTestFilePath (Phase 4)", () => {
    it("recognizes test dirs and .test/.spec suffixes", () => {
        assert.strictEqual(isTestFilePath("test/a.ts"), true);
        assert.strictEqual(isTestFilePath("tests/a.ts"), true);
        assert.strictEqual(isTestFilePath("src/a.test.ts"), true);
        assert.strictEqual(isTestFilePath("src/a.spec.js"), true);
    });

    it("rejects non-test paths", () => {
        assert.strictEqual(isTestFilePath("src/a.ts"), false);
        assert.strictEqual(isTestFilePath("src/contest.ts"), false);
    });
});
