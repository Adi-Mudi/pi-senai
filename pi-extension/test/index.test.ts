...
describe("coverage audit gaps", () => {
  ...
  it("runs legacy migrations on session_start, not on factory load", async () => {
    ...
    const ctx = {
      cwd: tmpDir,
      hasUI: true,
      ui: { notify: (msg: any) => notifications.push(String(msg)) },
    } as unknown as ExtensionContext;

    piSenaiExtension(api);

    // Factory load alone must stay side-effect free: nothing migrated yet.
    assert.ok(fs.existsSync(path.join(tmpDir, ".IDE_Plans", "orchestra")));
    assert.ok(fs.existsSync(path.join(tmpDir, ".IDE_Plans", "architect")));

    await handlers["session_start"]({}, ctx);

    assert.ok(
      notifications.some((l) => l.includes("Migrated 1 architecture file(s) to .pi/architect/.")),
      `expected the architect migration notice, got: ${notifications.join(" | ")}`,
    );
    assert.ok(
      notifications.some((l) => l.includes("Migrated legacy directories:")),
      `expected the orchestra dir migration notice, got: ${notifications.join(" | ")}`,
    );
  ...
  it("does not register the architect tools (owned by @adi-mudi/pi-chirpi)", () => {
    const tools: string[] = [];
    const api = makeApi();
    ...
    // senai_merge_architect_drivers and senai_finalize_architecture are
    // registered by pi-chirpi, not pi-senai — double registration would
    // conflict when both extensions are loaded.
