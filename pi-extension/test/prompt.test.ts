...
describe("prompt", () => {
  ...
  it("buildStagePrompt injects Mission type + Mission brief header lines when set", () => {
    const state = {
      ...makeState("planning", "run-bugfix"),
      missionType: "bugfix" as const,
      missionBriefPath: ".IDE_Plans/pi-senai/runs/run-bugfix/mission-brief.md",
    };
    const { prompt } = buildStagePrompt(tmpDir, state, "plan");
    assert.ok(prompt.includes("Mission type: bugfix"));
    assert.ok(
      prompt.includes("Mission brief: .IDE_Plans/pi-senai/runs/run-bugfix/mission-brief.md"),
    );
  });

  it("buildStagePrompt falls back to parsing the brief's ## Mission type section", () => {
    const runDir = path.join(tmpDir, ".IDE_Plans/pi-senai/runs/run-parse");
    ...
    const state = makeState("planning", "run-parse");
    const { prompt } = buildStagePrompt(tmpDir, state, "plan");
    assert.ok(prompt.includes("Mission type: bugfix"));
    assert.ok(prompt.includes(`Mission brief: ${path.join(runDir, "mission-brief.md")}`));
  });

  it("buildStagePrompt omits the Mission type / Mission brief lines when unknown", () => {
    const state = makeState("planning", "run-plain");
    const { prompt } = buildStagePrompt(tmpDir, state, "plan");
    assert.ok(!/(^|\n)Mission type: /.test(prompt), "no Mission type header line");
    assert.ok(!/(^|\n)Mission brief: /.test(prompt), "no Mission brief header line");
  });

  it("buildStagePrompt injects 'none locked' for the plan stage when no framework.json exists", () => {
    const state = makeState("planning", "run-1");
    const { prompt } = buildStagePrompt(tmpDir, state, "plan");
    assert.ok(prompt.includes("Framework lock: none locked"));
  });

  it("buildStagePrompt injects the locked framework id for the plan stage", () => {
    const lockDir = path.join(tmpDir, ".pi", "senai");
    fs.mkdirSync(lockDir, { recursive: true });
    fs.writeFileSync(
      path.join(lockDir, "framework.json"),
      JSON.stringify({ id: "express", source: "manual", setAt: new Date().toISOString() }),
    );
    const state = makeState("planning", "run-1");
    const { prompt } = buildStagePrompt(tmpDir, state, "plan");
    assert.ok(prompt.includes("Framework lock: express"));
    assert.ok(prompt.includes("MUST record `express`"));
  });

  it("buildStagePrompt treats a malformed framework.json as 'none locked' (never crashes)", () => {
    const lockDir = path.join(tmpDir, ".pi", "senai");
    fs.mkdirSync(lockDir, { recursive: true });
    fs.writeFileSync(path.join(lockDir, "framework.json"), "{ not json");
    const state = makeState("planning", "run-1");
    const { prompt } = buildStagePrompt(tmpDir, state, "plan");
    assert.ok(prompt.includes("Framework lock: none locked"));
  });

  it("buildStagePrompt omits the framework lock block outside the plan stage", () => {
    const state = makeState("implementing", "run-1");
    const { prompt } = buildStagePrompt(tmpDir, state, "implement");
    assert.ok(!prompt.includes("Framework lock:"));
  });
