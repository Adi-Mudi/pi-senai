...
describe("guardSpawnCall — web tool lock (hard block)", () => {
  function writeAgentWithTools(cwd: string, name: string, tools: string): void {
    const dir = path.join(cwd, ".pi", "agents");
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(
      path.join(dir, `${name}.md`),
      ["---", `name: ${name}`, "description: test agent", `tools: ${tools}`, "---", "", `# ${name}`].join("\n"),
      "utf8",
    );
  }

  function setupWebLockRun(cwd: string, agents: Record<string, string>): void {
    saveState(cwd, { ...defaultState(), currentStage: "planning", runId: "r1" });
    saveAgentConfig(cwd, { version: 1, agents });
  }

  it("hard-blocks a reviewer agent carrying WebSearch, naming role and tool", () => {
    const cwd = makeTmp();
    setupWebLockRun(cwd, { "reviewer-tests": "proj-reviewer-tests" });
    writeAgentWithTools(cwd, "proj-reviewer-tests", "read, write, WebSearch");
    const result = guardSpawnCall("subagent", { agent: "proj-reviewer-tests" }, cwd);
    assert.ok(result?.block, "web tool on a non-discussion role must block the spawn");
    assert.ok(result.reason.includes('"reviewer-tests"'), "reason names the role");
    assert.ok(result.reason.includes("WebSearch"), "reason names the offending tool");
    assert.ok(result.reason.includes("discussion"), "reason names the only allowed role");
  });

  it("hard-blocks on FetchURL too (case-insensitive)", () => {
    const cwd = makeTmp();
    setupWebLockRun(cwd, { implementer: "proj-implementer" });
    writeAgentWithTools(cwd, "proj-implementer", "read, write, edit, fetchurl");
    const result = guardSpawnCall("subagent", { agent: "proj-implementer" }, cwd);
    assert.ok(result?.block);
    assert.ok(result.reason.includes("fetchurl"));
  });

  it("allows the discussion role to carry web tools", () => {
    const cwd = makeTmp();
    setupWebLockRun(cwd, { discussion: "proj-discussion" });
    writeAgentWithTools(cwd, "proj-discussion", "read, write, WebSearch, FetchURL");
    assert.strictEqual(guardSpawnCall("subagent", { agent: "proj-discussion" }, cwd), undefined);
  });

  it("allows a custom-mapped agent without web tools", () => {
    const cwd = makeTmp();
    setupWebLockRun(cwd, { "reviewer-tests": "proj-reviewer-tests" });
    writeAgentWithTools(cwd, "proj-reviewer-tests", "read, write");
    assert.strictEqual(guardSpawnCall("subagent", { agent: "proj-reviewer-tests" }, cwd), undefined);
  });

  it("passes through when the mapped agent file does not exist", () => {
    const cwd = makeTmp();
    setupWebLockRun(cwd, { "reviewer-tests": "proj-reviewer-tests" });
    // No .pi/agents file written — the guard steps aside.
    assert.strictEqual(guardSpawnCall("subagent", { agent: "proj-reviewer-tests" }, cwd), undefined);
  });

  it("skips roles mapped to their default (same skip rule as doctor)", () => {
    const cwd = makeTmp();
    setupWebLockRun(cwd, { "reviewer-tests": "reviewer" });
    // Even if a project agent named 'reviewer' exists with web tools, the
    // role is not custom-mapped, so the lock does not apply.
    writeAgentWithTools(cwd, "reviewer", "read, WebSearch");
    assert.strictEqual(guardSpawnCall("subagent", { agent: "reviewer" }, cwd), undefined);
  });

  it("still blocks the bare-name collision before any web-tool check", () => {
    const cwd = makeTmp();
    setupWebLockRun(cwd, { "reviewer-tests": "proj-reviewer-tests" });
    writeAgentWithTools(cwd, "proj-reviewer-tests", "read, write, WebSearch");
    const result = guardSpawnCall("subagent", { agent: "reviewer-tests" }, cwd);
    assert.ok(result?.block);
    assert.ok(result.reason.includes("bare role/built-in name"), "bare-name block wins");
  });
});
