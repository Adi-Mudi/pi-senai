---
id: generic
name: Generic (technology-agnostic)
keywords: ["generic", "fallback", "any"]
---

# Generic Technology Resource

Technology-agnostic craft. Used when no specific technology resource matches. Sources are language-neutral official references where applicable.

## Core rules

1. Keep functions small and single-purpose; pass dependencies explicitly, no hidden globals.
2. Handle errors at the boundary: catch, log with context, and fail visibly instead of silently.
3. Validate all external input (files, network, user data) before use.
4. Keep secrets out of source code; use environment variables or a secret store.
